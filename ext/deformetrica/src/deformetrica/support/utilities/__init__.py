...
def get_torch_integer_type(dtype):
    ...
    return {
        ...
        "torch.float64": torch.LongTensor,
    }[dtype]
...
def move_data(data, device="cpu", dtype=None, requires_grad=None):
    ...
    assert isinstance(data, torch.Tensor), "Expecting Torch.Tensor instance not " + str(
        type(data)
    )
...
def convert_deformable_object_to_torch(deformable_object, device="cpu"):
    ...
    if not isinstance(deformable_object.bounding_box, torch.Tensor):
        deformable_object.bounding_box = move_data(
            deformable_object.bounding_box, device=device
        )
    ...
    for i, _ in enumerate(deformable_object.object_list):
        if hasattr(deformable_object.object_list[i], "bounding_box"):
            deformable_object.object_list[i].bounding_box = move_data(
                deformable_object.object_list[i].bounding_box, device=device
            )

        if hasattr(deformable_object.object_list[i], "connectivity"):
            deformable_object.object_list[i].connectivity = move_data(
                deformable_object.object_list[i].connectivity, device=device
            )

        if hasattr(deformable_object.object_list[i], "points"):
            deformable_object.object_list[i].points = move_data(
                deformable_object.object_list[i].points, device=device
            )

        if hasattr(deformable_object.object_list[i], "affine"):
            deformable_object.object_list[i].affine = move_data(
                deformable_object.object_list[i].affine, device=device
            )

        if hasattr(deformable_object.object_list[i], "corner_points"):
            deformable_object.object_list[i].corner_points = move_data(
                deformable_object.object_list[i].corner_points, device=device
            )

        if hasattr(deformable_object.object_list[i], "intensities"):
            deformable_object.object_list[i].intensities = move_data(
                deformable_object.object_list[i].intensities, device=device
            )
...
def get_device_from_string(device):
    ...
    if isinstance(device, str) and device.startswith("cuda") and ":" in device:
        ...
        torch_device, device_id = (
            torch.device(type=device_type, index=int(device_id)),
            int(device_id),
        )
...
def get_best_device(gpu_mode=GpuMode.AUTO):
    ...
    device = (
        "cuda:" + str(device_id) if use_cuda and torch.cuda.is_available() else "cpu"
    )
