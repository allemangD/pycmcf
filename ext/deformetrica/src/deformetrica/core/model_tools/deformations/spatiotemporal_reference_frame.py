...
class SpatiotemporalReferenceFrame:
    ...
    def set_use_rk2_for_shoot(
        self, flag
    ):  # Cannot modify the shoot integration of the geodesic, which require rk2.
    ...
    def get_template_points_exponential_parameters(self, time, sources):
        ...
        if len(self.times) == 1:
            ...
            initial_template_points = {
                key: value[0] for key, value in self.template_points_t.items()
            }
            initial_control_points = self.control_points_t[0]
            initial_momenta = torch.mm(
                self.projected_modulation_matrix_t[0], sources.unsqueeze(1)
            ).view(self.geodesic.momenta_t0.size())

        # Standard case.
        else:
            index, weight_left, weight_right = (
                self._get_interpolation_index_and_weights(time)
            )
            template_points = {
            ...
            control_points = (
                weight_left * self.control_points_t[index - 1]
                + weight_right * self.control_points_t[index]
            )
            modulation_matrix = (
                weight_left * self.projected_modulation_matrix_t[index - 1]
    ...
    def get_template_points(self, time, sources, device=None):
        ...
        if len(self.times) == 1:
            ...
            self.exponential.set_initial_momenta(
                torch.mm(
                    self.projected_modulation_matrix_t[0], sources.unsqueeze(1)
                ).view(self.geodesic.momenta_t0.size())
            )
    ...
    def update(self):
        ...
        if self.transport_is_modified:
            ...
            self.projected_modulation_matrix_t = [
                torch.zeros(
                    self.projected_modulation_matrix_t0.size(),
            ...
            for s in range(self.number_of_sources):
                space_shift_t0 = (
                    self.projected_modulation_matrix_t0[:, s]
                    .contiguous()
                    .view(self.geodesic.momenta_t0.size())
                )
            ...
            for s in range(self.number_of_sources):
                space_shift_t = [
                    elt[:, s].contiguous().view(self.geodesic.momenta_t0.size())
                ...
                for t, space_shift in enumerate(space_shift_t):
                    projected_modulation_matrix_t_extended[t][:, s] = space_shift.view(
                        -1
                    )
    ...
    def _update_projected_modulation_matrix_t0(self, device="cpu"):
        self.projected_modulation_matrix_t0 = torch.zeros(
            self.modulation_matrix_t0.size(),
            dtype=self.modulation_matrix_t0.dtype,
            device=device,
        )

        norm_squared = self.geodesic.backward_exponential.scalar_product(
            self.geodesic.control_points_t0,
            self.geodesic.momenta_t0,
            self.geodesic.momenta_t0,
        )

        for s in range(self.number_of_sources):
            space_shift_t0 = (
                self.modulation_matrix_t0[:, s]
                .contiguous()
                .view(self.geodesic.momenta_t0.size())
            )
            sp = (
                self.geodesic.backward_exponential.scalar_product(
                    self.geodesic.control_points_t0,
                    self.geodesic.momenta_t0,
                    space_shift_t0,
                )
            ...
            self.projected_modulation_matrix_t0[:, s] = projected_space_shift_t0.view(
                -1
            ).contiguous()
    ...
    def write(
        ...
        self.exponential.number_of_time_points = 1 + 3 * (
            self.exponential.number_of_time_points - 1
        )
        for s in range(self.number_of_sources):
            # Direct flow.
            space_shift = (
            ...
            self.exponential.set_initial_template_points(
                self.geodesic.template_points_t0
            )
            ...
            for j in range(self.exponential.number_of_time_points):
                ...
                for k, (object_name, object_extension) in enumerate(
                    ...
                    name = (
                        ...
                        + object_name
                        + "__"
                        + str(self.exponential.number_of_time_points - 1 + j)
                        + (
                            "__+%.2f_sigma"
                            % (
                                3.0
                ...
                deformed_data = template.get_deformed_data(
        ...
        self.exponential.number_of_time_points = (
            1 + (self.exponential.number_of_time_points - 1) // 3
        )

        # Optionally write the projected modulation matrices along the geodesic flow -----------------------------------
        if write_adjoint_parameters:
            times = self.geodesic.get_times()
            for t, (time, modulation_matrix) in enumerate(
                zip(times, self.projected_modulation_matrix_t)
            ):
                write_2D_array(
        ...
        if write_exponential_flow:
            ...
            for t, (time, modulation_matrix) in enumerate(
                ...
                for s in range(self.number_of_sources):
                    ...
                    self.exponential.set_initial_template_points(
                        {key: value[t] for key, value in self.template_points_t.items()}
                    )
                    self.exponential.set_initial_control_points(
                        self.control_points_t[t]
                    )
                    self.exponential.set_initial_momenta(space_shift)
                    self.exponential.update()
