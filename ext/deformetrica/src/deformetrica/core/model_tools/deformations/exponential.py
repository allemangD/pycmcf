...
class Exponential:
    ...
    def __init__(
        ...
        control_points_t=None,
        initial_momenta=None,
        momenta_t=None,
        initial_template_points=None,
        template_points_t=None,
        shoot_is_modified=True,
        flow_is_modified=True,
        ...
        if shoot_kernel_type is not None:
            self.shoot_kernel = kernel_factory.factory(
                shoot_kernel_type,
                gpu_mode=kernel.gpu_mode,
    ...
    def move_data_to_(self, device):
        if self.initial_control_points is not None:
            self.initial_control_points = utilities.move_data(
                self.initial_control_points, device
            )
        ...
        if self.initial_template_points is not None:
            self.initial_template_points = {
                key: utilities.move_data(value, device)
                for key, value in self.initial_template_points.items()
            }
    ...
    def light_copy(self):
        light_copy = Exponential(
            ...
            self.shoot_kernel.kernel_type,
            self.number_of_time_points,
            self.initial_control_points,
            self.control_points_t,
            self.initial_momenta,
            self.momenta_t,
            self.initial_template_points,
            self.template_points_t,
            self.shoot_is_modified,
            self.flow_is_modified,
            self.use_rk2_for_shoot,
            self.use_rk2_for_flow,
        )
        return light_copy
    ...
    def get_template_points(self, time_index=None):
        ...
        if self.flow_is_modified:
            msg = (
                "You tried to get some template points, but the flow was modified. "
    ...
    def get_norm_squared(self):
        return self.scalar_product(
            self.initial_control_points, self.initial_momenta, self.initial_momenta
        )
    ...
    def shoot(self):
        ...
        assert len(self.initial_control_points) > 0, (
            "Control points not initialized in shooting"
        )
        ...
        if self.use_rk2_for_shoot:
            ...
            for i in range(self.number_of_time_points - 1):
                new_cp, new_mom = self._euler_step(
                    self.shoot_kernel, self.control_points_t[i], self.momenta_t[i], dt
                )
                self.control_points_t.append(new_cp)
                self.momenta_t.append(new_mom)
    ...
    def flow(self):
        ...
        assert not self.shoot_is_modified, (
            "CP or momenta were modified and the shoot not computed, and now you are asking me to flow ?"
        )
        ...
        if "landmark_points" in self.initial_template_points.keys():
            ...
            for i in range(self.number_of_time_points - 1):
                d_pos = self.kernel.convolve(
                    landmark_points[i], self.control_points_t[i], self.momenta_t[i]
                )
                ...
                if self.use_rk2_for_flow:
                    ...
                    if i < self.number_of_time_points - 2:
                        ...
                        landmark_points[-1] = landmark_points[i] + dt / 2 * (
                            self.kernel.convolve(
                                landmark_points[i + 1], final_cp, final_mom
                            )
                            + d_pos
                        )
        ...
        if "image_points" in self.initial_template_points.keys():
            ...
            for i in range(self.number_of_time_points - 1):
                vf = self.kernel.convolve(
                    image_points[0].contiguous().view(-1, dimension),
                    self.control_points_t[i],
                    self.momenta_t[i],
                ).view(image_shape)
                dY = self._compute_image_explicit_euler_step_at_order_1(
                    image_points[i], vf
                )
    ...
    def parallel_transport(
        self, momenta_to_transport, initial_time_point=0, is_orthogonal=False
    ):
        """
        ...
        # Sanity checks ------------------------------------------------------------------------------------------------
        assert not self.shoot_is_modified, (
            "You want to parallel transport but the shoot was modified, please update."
        )
        assert self.use_rk2_for_shoot, (
            "The shoot integration must be done with a second order numerical scheme in order to use parallel transport."
        )
        ...
        if (
            torch.norm(self.initial_momenta).detach().cpu().numpy() < 1e-6
            or torch.norm(momenta_to_transport).detach().cpu().numpy() < 1e-6
        ):
            parallel_transport_t = [momenta_to_transport] * (
                self.number_of_time_points - initial_time_point
            )
        ...
        if not is_orthogonal:
            sp = (
                self.scalar_product(
                    self.control_points_t[initial_time_point],
                    momenta_to_transport,
                    self.momenta_t[initial_time_point],
                )
                / norm_squared
            )
            momenta_to_transport_orthogonal = (
                momenta_to_transport - sp * self.momenta_t[initial_time_point]
            )
        ...
        initial_norm_squared = self.scalar_product(
            self.control_points_t[initial_time_point],
        ...
        for i in range(initial_time_point, self.number_of_time_points - 1):
            ...
            cp_eps_pos = self._rk2_step(
                self.shoot_kernel,
                self.control_points_t[i],
                self.momenta_t[i] + epsilon * parallel_transport_t[-1],
                h,
                return_mom=False,
            )
            cp_eps_neg = self._rk2_step(
                self.shoot_kernel,
                self.control_points_t[i],
                self.momenta_t[i] - epsilon * parallel_transport_t[-1],
                h,
            ...
            if i not in self.cometric_matrices:
                kernel_matrix = self.shoot_kernel.get_kernel_matrix(
                    self.control_points_t[i + 1]
                )
            ...
            scalar_prod_with_velocity = (
                self.scalar_product(
                    self.control_points_t[i + 1], approx_momenta, self.momenta_t[i + 1]
                )
                / norm_squared
            )
            ...
            # Renormalization ------------------------------------------------------------------------------------------
            approx_momenta_norm_squared = self.scalar_product(
                self.control_points_t[i + 1], approx_momenta, approx_momenta
            )
            ...
            if abs(renormalization_factor.detach().cpu().numpy() - 1.0) > 0.1:
                ...
                worst_renormalization_factor = (
                    renormalization_factor.detach().cpu().numpy()
                )
        ...
        assert (
            len(parallel_transport_t) == self.number_of_time_points - initial_time_point
    ...
    def extend(self, number_of_additional_time_points):
        ...
        for i in range(number_of_additional_time_points):
            if self.use_rk2_for_shoot:
                ...
                new_cp, new_mom = self._euler_step(
                    self.kernel, self.control_points_t[-1], self.momenta_t[-1], dt
                )
        ...
        # Standard case.
            ...
            if "landmark_points" in self.initial_template_points.keys():
                for ii in range(number_of_additional_time_points):
                    ...
                    d_pos = self.kernel.convolve(
                        self.template_points_t["landmark_points"][i],
                        self.control_points_t[i],
                        self.momenta_t[i],
                    )
            ...
            if "image_points" in self.initial_template_points.keys():
                ...
                for ii in range(number_of_additional_time_points):
                    ...
                    vf = self.kernel.convolve(
                        self.initial_template_points["image_points"]
                        .contiguous()
                        .view(-1, dimension),
                        self.control_points_t[i],
                        self.momenta_t[i],
                    ...
                    dY = self._compute_image_explicit_euler_step_at_order_1(
                        self.template_points_t["image_points"][i], vf
                    )
                    self.template_points_t["image_points"].append(
                        self.template_points_t["image_points"][i] - dt * dY
                    )
        ...
        length_ratio = float(
            self.number_of_time_points + number_of_additional_time_points - 1
    ...
    def _euler_step(kernel, cp, mom, h):
        ...
        """
        assert cp.device == mom.device, (
            "tensors must be on the same device, cp.device="
            + str(cp.device)
            + ", mom.device="
            + str(mom.device)
        )
        return cp + h * kernel.convolve(
            cp, cp, mom
    ...
    def _rk2_step(kernel, cp, mom, h, return_mom=True):
        ...
        mid_cp = cp + h / 2.0 * kernel.convolve(cp, cp, mom)
        mid_mom = mom - h / 2.0 * kernel.convolve_gradient(mom, cp)
        if return_mom:
            return cp + h * kernel.convolve(
                mid_cp, mid_cp, mid_mom
    ...
    def _compute_image_explicit_euler_step_at_order_1(Y, vf):
        assert Y.device == vf.device, (
            "tensors must be on the same device, Y.device="
            + str(Y.device)
            + ", vf.device="
            + str(vf.device)
        )
        ...
        if dimension == 2:
            ...
            # Center.
            dY[1 : ni - 1, :] = dY[1 : ni - 1, :] + 0.5 * vf[1 : ni - 1, :, 0].view(
                ni - 2, nj, 1
            ).expand(ni - 2, nj, 2) * (Y[2:ni, :] - Y[0 : ni - 2, :])
            dY[:, 1 : nj - 1] = dY[:, 1 : nj - 1] + 0.5 * vf[:, 1 : nj - 1, 1].view(
                ni, nj - 2, 1
            ).expand(ni, nj - 2, 2) * (Y[:, 2:nj] - Y[:, 0 : nj - 2])

            # Borders.
            dY[0, :] = dY[0, :] + vf[0, :, 0].view(nj, 1).expand(nj, 2) * (
                Y[1, :] - Y[0, :]
            )
            dY[ni - 1, :] = dY[ni - 1, :] + vf[ni - 1, :, 0].view(nj, 1).expand(
                nj, 2
            ) * (Y[ni - 1, :] - Y[ni - 2, :])

            dY[:, 0] = dY[:, 0] + vf[:, 0, 1].view(ni, 1).expand(ni, 2) * (
                Y[:, 1] - Y[:, 0]
            )
            dY[:, nj - 1] = dY[:, nj - 1] + vf[:, nj - 1, 1].view(ni, 1).expand(
                ni, 2
            ) * (Y[:, nj - 1] - Y[:, nj - 2])
            ...
            # Center.
            dY[1 : ni - 1, :, :] = dY[1 : ni - 1, :, :] + 0.5 * vf[
                1 : ni - 1, :, :, 0
            ].view(ni - 2, nj, nk, 1).expand(ni - 2, nj, nk, 3) * (
                Y[2:ni, :, :] - Y[0 : ni - 2, :, :]
            )
            dY[:, 1 : nj - 1, :] = dY[:, 1 : nj - 1, :] + 0.5 * vf[
                :, 1 : nj - 1, :, 1
            ].view(ni, nj - 2, nk, 1).expand(ni, nj - 2, nk, 3) * (
                Y[:, 2:nj, :] - Y[:, 0 : nj - 2, :]
            )
            dY[:, :, 1 : nk - 1] = dY[:, :, 1 : nk - 1] + 0.5 * vf[
                :, :, 1 : nk - 1, 2
            ].view(ni, nj, nk - 2, 1).expand(ni, nj, nk - 2, 3) * (
                Y[:, :, 2:nk] - Y[:, :, 0 : nk - 2]
            )

            # Borders.
            dY[0, :, :] = dY[0, :, :] + vf[0, :, :, 0].view(nj, nk, 1).expand(
                nj, nk, 3
            ) * (Y[1, :, :] - Y[0, :, :])
            dY[ni - 1, :, :] = dY[ni - 1, :, :] + vf[ni - 1, :, :, 0].view(
                nj, nk, 1
            ).expand(nj, nk, 3) * (Y[ni - 1, :, :] - Y[ni - 2, :, :])

            dY[:, 0, :] = dY[:, 0, :] + vf[:, 0, :, 1].view(ni, nk, 1).expand(
                ni, nk, 3
            ) * (Y[:, 1, :] - Y[:, 0, :])
            dY[:, nj - 1, :] = dY[:, nj - 1, :] + vf[:, nj - 1, :, 1].view(
                ni, nk, 1
            ).expand(ni, nk, 3) * (Y[:, nj - 1, :] - Y[:, nj - 2, :])

            dY[:, :, 0] = dY[:, :, 0] + vf[:, :, 0, 2].view(ni, nj, 1).expand(
                ni, nj, 3
            ) * (Y[:, :, 1] - Y[:, :, 0])
            dY[:, :, nk - 1] = dY[:, :, nk - 1] + vf[:, :, nk - 1, 2].view(
                ni, nj, 1
            ).expand(ni, nj, 3) * (Y[:, :, nk - 1] - Y[:, :, nk - 2])
    ...
    def write_flow(
        ...
        for j in range(self.number_of_time_points):
            ...
            if write_adjoint_parameters:
                ...
                write_2D_array(
                    cp, output_dir, elt + "__ControlPoints__tp_" + str(j) + ".txt"
                )
                write_3D_array(
                    mom, output_dir, elt + "__Momenta__tp_" + str(j) + ".txt"
                )
    ...
    def write_control_points_and_momenta_flow(self, name):
        ...
        for j, (control_points, momenta) in enumerate(
            zip(self.control_points_t, self.momenta_t)
        ):
            write_2D_array(
                control_points.detach().cpu().numpy(),
                name + "__control_points_" + str(j) + ".txt",
            )
            write_2D_array(
                momenta.detach().cpu().numpy(), name + "__momenta_" + str(j) + ".txt"
            )
