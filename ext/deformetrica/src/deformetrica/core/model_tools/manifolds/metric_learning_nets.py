        ...
        for layer in self.layers:
            try:
                if layer.weight is not None:
                    out[pos : pos + len(layer.weight.view(-1))] = (
                        layer.weight.grad.view(-1).cpu().data.numpy()
                    )
            ...
            try:
                if layer.bias is not None:
                    out[pos : pos + len(layer.bias.view(-1))] = (
                        layer.bias.grad.view(-1).cpu().data.numpy()
                    )
    ...
    def set_parameters(self, nn_parameters):
        ...
        for layer in self.layers:
            try:
                if layer.weight is not None:
                    layer.weight.data = (
                        nn_parameters[pos : pos + len(layer.weight.view(-1))]
            ...
            try:
                if layer.bias is not None:
                    layer.bias.data = (
                        nn_parameters[pos : pos + len(layer.bias.view(-1))]
        ...
        for layer in self.layers:
            try:
                if layer.weight is not None:
                    ...
                    out[pos : pos + len(layer.weight.view(-1))] = (
                        layer.weight.view(-1).cpu().data.numpy()
                    )
                    pos += len(layer.weight.view(-1))
            except AttributeError:
                pass
            try:
                if layer.bias is not None:
                    out[pos : pos + len(layer.bias.view(-1))] = (
                        layer.bias.view(-1).cpu().data.numpy()
                    )
                    pos += len(layer.bias.view(-1))
            except AttributeError:
                pass
...
class DiffeoNet(AbstractNet):
    def __init__(self, in_dimension=10, nb_cp=10):
        ...
        self.elu1 = nn.ELU()
        self.fc2 = nn.Linear(
            2 * nb_cp * Settings().dimension, 2 * nb_cp * Settings().dimension
        )
        self.elu2 = nn.ELU()
        self.tanh = nn.Tanh()
        self.update()
