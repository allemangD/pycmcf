...
class PointCloud(Landmark):
    ...
    def __init__(self, points, weights=None):
        ...
        self.centers, self.normals = PointCloud._get_centers_and_normals(
    ...
    def _get_centers_and_normals(
        ...
        normals = utilities.move_data(
            torch.ones(points.size(0), 1) / points.size(0),
            device=device,
    ...
    def get_centers_and_normals(
        ...
        if points is None:
            if not self.is_modified:
                ...
                logger.debug(
                    "Call of PointCloud.get_centers_and_normals with is_modified=True flag."
                )
