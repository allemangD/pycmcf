...
from deformetrica.core.observations.deformable_objects.landmarks.landmark import (
    Landmark,
)
...
class SurfaceMesh(Landmark):
    ...
    def _get_centers_and_normals(
        ...
        triangles = utilities.move_data(
            triangles, dtype=tensor_integer_type, device=device
        )
    ...
    def get_centers_and_normals(
        ...
        if points is None:
            if not self.is_modified:
                return (
                    utilities.move_data(
                        self.centers, dtype=tensor_scalar_type, device=device
                    ),
                    utilities.move_data(
                        self.normals, dtype=tensor_scalar_type, device=device
                    ),
                )

            else:
                logger.debug(
                    "Call of SurfaceMesh.get_centers_and_normals with is_modified=True flag."
                )
                points = torch.from_numpy(self.points)

        return SurfaceMesh._get_centers_and_normals(
            points,
            torch.from_numpy(self.connectivity),
