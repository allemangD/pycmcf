...
class McmcSaem(AbstractEstimator):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.sampler = SrwMhwgSampler(
            individual_proposal_distributions=individual_proposal_distributions
        )
        ...
        self.memory_window_size = (
            1  # Size of the averaging window for the acceptance rates.
        )

        self.number_of_trajectory_points = min(self.max_iterations, 500)
        self.save_model_parameters_every_n_iters = max(
            1, int(self.max_iterations / float(self.number_of_trajectory_points))
        )
        ...
        if load_state_file:
            ...
            logger.info(
                "State file loaded, it was at iteration %d." % self.current_iteration
            )
            ...
            self.current_acceptance_rates_in_window = (
                None  # Memory of the last memory_window_size acceptance rates.
            )
            self.average_acceptance_rates_in_window = (
                None  # Moving average of current_acceptance_rates_in_window.
            )
            self.model_parameters_trajectory = (
                None  # Memory of the model parameters along the estimation.
            )
            self.individual_random_effects_samples_stack = (
                None  # Stack of the last individual random effect samples.
            )
    ...
    def update(self):
        ...
        logger.info(
            ">> MCMC-SAEM algorithm launched for "
        ...
        averaged_population_RER = {
            key: np.zeros(value.shape) for key, value in self.population_RER.items()
        }
        averaged_individual_RER = {
            key: np.zeros(value.shape) for key, value in self.individual_RER.items()
        }
        ...
        while self.callback_ret and self.current_iteration < self.max_iterations:
            ...
            for n in range(self.sample_every_n_mcmc_iters):
                ...
                self.current_acceptance_rates, current_model_terms = (
                    self.sampler.sample(
            ...
            fixed_effects_before_maximization = (
                self.statistical_model.get_fixed_effects()
            )
            self._maximize_over_fixed_effects()
            fixed_effects_after_maximization = (
                self.statistical_model.get_fixed_effects()
            )
            fixed_effects = {
                key: value + step * (fixed_effects_after_maximization[key] - value)
            ...
            if step < 1.0:
                coefficient_1 = float(
                    self.current_iteration + 1 - self.number_of_burn_in_iterations
                )
    ...
    def _maximize_over_fixed_effects(self):
        ...
        if (
            self.statistical_model.has_maximization_procedure is not None
    ...
    def _initialize_sufficient_statistics(self):
        ...
        self.sufficient_statistics = {
            key: np.zeros(value.shape) for key, value in sufficient_statistics.items()
        }
    ...
    def _initialize_model_parameters_trajectory(self):
        ...
        for key, value in self.statistical_model.get_fixed_effects(mode="all").items():
            self.model_parameters_trajectory[key] = np.zeros(
                (self.number_of_trajectory_points + 1, value.size)
            )
    ...
    def _initialize_individual_random_effects_samples_stack(self):
        number_of_concentration_iterations = (
            self.max_iterations - self.number_of_burn_in_iterations
        )
        self.individual_random_effects_samples_stack = {}
        for key, value in self.individual_RER.items():
            if number_of_concentration_iterations > 0:
                ...
                self.individual_random_effects_samples_stack[key][0, :] = (
                    value.flatten()
                )
    ...
    def _update_individual_random_effects_samples_stack(self):
        for key, value in self.individual_RER.items():
            self.individual_random_effects_samples_stack[key][
                self.current_iteration - self.number_of_burn_in_iterations - 1, :
            ] = value.flatten()
