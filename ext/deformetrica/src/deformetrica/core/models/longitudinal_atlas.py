...
def compute_exponential_and_attachment(args):
    ...
    assert (
        len(ijs)
        == len(initial_template_points)
        == len(initial_control_points)
        == len(initial_momenta)
        == len(targets)
    ...
    for i in range(len(ijs)):
        ...
        if with_grad:
            exponential.initial_template_points = {
                key: value.requires_grad_()
                for key, value in exponential.initial_template_points.items()
            }
        ...
        residual = multi_object_attachment.compute_distances(
            deformed_data, template, target
        )
        ...
        if with_grad:
            ...
            ret_grad_control_points.append(
                exponential.initial_control_points.grad.cpu()
            )
...
class LongitudinalAtlas(AbstractStatisticalModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        AbstractStatisticalModel.__init__(
            self,
            name="LongitudinalAtlas",
        ...
        if gpu_mode not in [GpuMode.KERNEL]:
            logger.warning(
                "LongitudinalAtlas model currently only accepts KERNEL gpu mode. Forcing KERNEL gpu mode."
            )
        ...
        self.is_frozen = {
            ...
            "momenta": freeze_momenta,
            "modulation_matrix": freeze_modulation_matrix,
            "reference_time": freeze_reference_time,
            "time_shift_variance": freeze_time_shift_variance,
        ...
        self.individual_random_effects["acceleration"] = (
            MultiScalarTruncatedNormalDistribution()
        )
        ...
        self.objects_noise_variance_prior_normalized_dof = [
            elt["noise_variance_prior_normalized_dof"]
            for elt in template_specifications.values()
        ]
        self.objects_noise_variance_prior_scale_std = [
            elt["noise_variance_prior_scale_std"]
            for elt in template_specifications.values()
        ]
        ...
        self.individual_random_effects["sources"].set_mean(
            np.zeros((self.number_of_sources,))
        )
        ...
        if initial_acceleration_mean is None:
            ...
            self.individual_random_effects["acceleration"].set_mean(
                np.ones((1,)) * acceleration_mean
            )
        ...
        **kwargs,
        ...
        individual_RER = {
            "sources": initialize_sources(
                initial_sources, number_of_subjects, self.number_of_sources
            ),
            "onset_age": initialize_onset_ages(
                initial_onset_ages, number_of_subjects, self.get_reference_time()
            ),
            "acceleration": initialize_accelerations(
                initial_accelerations, number_of_subjects
            ),
        }
    ...
    def initialize_noise_variance(self, dataset, individual_RER):
        ...
        for k, normalized_dof in enumerate(
            self.objects_noise_variance_prior_normalized_dof
        ):
            dof = (
                dataset.total_number_of_observations
                * normalized_dof
                * self.objects_noise_dimension[k]
            )
        ...
        if np.min(self.fixed_effects["noise_variance"]) < 0.0:
            ...
            sources, onset_ages, accelerations = self._individual_RER_to_torch_tensors(
                individual_RER, False
            )
            absolute_times, tmin, tmax = self._compute_absolute_times(
            ...
            residuals, _, _ = self._compute_residuals(
                dataset, template_data, absolute_times, sources
            )
            ...
            self.fixed_effects["noise_variance"] = np.array(
                self.priors["noise_variance"].scale_scalars
            )

        else:
            for k, object_noise_variance in enumerate(
                self.fixed_effects["noise_variance"]
            ):
                self.priors["noise_variance"].scale_scalars.append(
                    object_noise_variance
                )
    ...
    def __initialize_control_points_prior(self):
        ...
        if not self.is_frozen["control_points"]:
            ...
            self.priors["control_points"].set_variance_sqrt(
                self.spatiotemporal_reference_frame.get_kernel_width()
            )
    ...
    def __initialize_momenta_prior(self):
        ...
        if not self.is_frozen["momenta"]:
            ...
            rkhs_matrix = initialize_covariance_momenta_inverse(
                ...
                self.spatiotemporal_reference_frame.exponential.kernel,
                self.dimension,
            )
            self.priors["momenta"].set_variance(
    ...
    def __initialize_modulation_matrix_prior(self):
        ...
        if not self.is_frozen["modulation_matrix"]:
            ...
            self.priors["modulation_matrix"].set_variance_sqrt(
                self.spatiotemporal_reference_frame.get_kernel_width()
            )
    ...
    def __initialize_reference_time_prior(self, initial_time_shift_variance):
        ...
        if not self.is_frozen["reference_time"]:
            ...
            self.priors["reference_time"].set_mean(
                np.zeros((1,)) + self.get_reference_time()
            )
    ...
    def __initialize_time_shift_variance_prior(self):
        ...
        if not self.is_frozen["time_shift_variance"]:
            ...
            self.priors["time_shift_variance"].scale_scalars.append(
                self.get_time_shift_variance()
            )
    ...
    def __initialize_acceleration_variance_prior(self):
        ...
        if not self.is_frozen["acceleration_variance"]:
            ...
            self.priors["acceleration_variance"].scale_scalars.append(
                self.get_acceleration_variance()
            )
    ...
    def compute_log_likelihood(
        ...
        sources, onset_ages, accelerations = self._individual_RER_to_torch_tensors(
            individual_RER, with_grad and mode == "complete", device=device
        )
        ...
        residuals, checkpoints_tensors, grad_checkpoints_tensors = (
        ...
        attachments, grad_checkpoints_tensors = self._compute_individual_attachments(
            residuals, grad_checkpoints_tensors
        )
        attachment = torch.sum(attachments)

        # Compute the regularity terms according to the mode.
        regularity = utilities.move_data(
            np.array(0.0), dtype=self.tensor_scalar_type, device=device
        )
        if mode == "complete":
            regularity = self._compute_random_effects_regularity(
                sources, onset_ages, accelerations, device=device
            )
            regularity += self._compute_class1_priors_regularity()
        if mode in ["complete", "class2"]:
            regularity += self._compute_class2_priors_regularity(
                template_data, control_points, momenta, modulation_matrix
            )
        ...
        if with_grad:
            ...
            if self.number_of_processes == 1:
                ...
                torch.autograd.backward(
                    ...
                    + [
                        torch.ones(
                            regularity.size(),
                            device=regularity.device,
                            dtype=regularity.dtype,
                        )
                    ],
                )

            logger.debug(
                "time taken for backwards: " + str(time.perf_counter() - start)
            )
            ...
            gradient = {
                key: value.detach().cpu().numpy() for key, value in gradient.items()
            }
    ...
    def compute_sufficient_statistics(
        ...
        if (not self.is_frozen["reference_time"]) or (
            not self.is_frozen["time_shift_variance"]
        ):
        ...
        if not self.is_frozen["noise_variance"]:
            ...
            if model_terms is not None and self.number_of_objects == 1:
                sufficient_statistics["S4"][0] += (
                    -2 * np.sum(model_terms) * self.get_noise_variance()
                )
            ...
            if residuals is None:
                ...
                sources, onset_ages, accelerations = (
                ...
                residuals, _, _ = self._compute_residuals(
                    dataset, template_data, absolute_times, sources, with_grad=False
                )

            for i in range(len(residuals)):
                for j in range(len(residuals[i])):
                    for k in range(self.number_of_objects):
                        sufficient_statistics["S4"][k] += (
                            residuals[i][j][k].detach().cpu().numpy()
                        )
    ...
    def update_fixed_effects(self, dataset, sufficient_statistics):
        ...
        if (not self.is_frozen["reference_time"]) and (
            not self.is_frozen["time_shift_variance"]
        ):
            ...
            tshiftvar_prior_dof = self.priors["time_shift_variance"].degrees_of_freedom[
                0
            ]
            ...
            if iteration == max_number_of_iterations:
                msg = (
                    "In longitudinal_atlas.update_fixed_effects, the intricate update of the reference time and "
            ...
            time_shift_variance = (
                sufficient_statistics["S2"]
                - 2 * reftime * sufficient_statistics["S1"]
                + number_of_subjects * reftime**2
                + tshiftvar_prior_dof * tshiftvar_prior_scale
            ) / (number_of_subjects + tshiftvar_prior_dof)
        ...
        if not self.is_frozen["acceleration_variance"]:
            ...
            for iteration in range(max_number_of_iterations):
                ...
                std_new = 1.0 / math.sqrt(
                    (number_of_subjects * (1 - (phi / std_old) / (1 - Phi)) + prior_dof)
                ...
                if iteration == max_number_of_iterations:
                    msg = (
                        "When updating the acceleration std parameter from the empirical std, the fixed-point "
        ...
        if not self.is_frozen["noise_variance"]:
            ...
            for k in range(self.number_of_objects):
                noise_variance[k] = (
                    sufficient_statistics["S4"][k]
                    + prior_scale_scalars[k] * prior_dofs[k]
                ) / float(
                    total_number_of_observations * self.objects_noise_dimension[k]
                    + prior_dofs[k]
                )
    ...
    def preoptimize(self, dataset, individual_RER):
        ...
        expected_mean_acceleration = self.individual_random_effects[
            "acceleration"
        ].get_expected_mean()
        ...
        self.set_momenta(
            self.get_momenta() * mean_acceleration / expected_mean_acceleration
        )
    ...
    def _compute_individual_attachments(self, residuals, grad_checkpoints_tensors=None):
        ...
        attachments = torch.zeros(
            (number_of_subjects,), dtype=self.tensor_scalar_type.dtype, device=device
        )
        noise_variance = utilities.move_data(
            self.fixed_effects["noise_variance"],
        ...
        if self.number_of_processes > 1:
            ...
            grad_checkpoints_tensors = [
                -0.5 * elt / noise_variance for elt in grad_checkpoints_tensors
            ]

        return attachments, grad_checkpoints_tensors

    def _compute_random_effects_regularity(
        self, sources, onset_ages, accelerations, device="cpu"
    ):
        """
        Fully torch.
        """
    ...
    def _compute_class1_priors_regularity(self):
        ...
        if not self.is_frozen["reference_time"]:
            regularity += self.priors["reference_time"].compute_log_likelihood(
                self.fixed_effects["reference_time"]
            )

        # Time-shift variance prior (if not frozen).
        if not self.is_frozen["time_shift_variance"]:
            regularity += self.priors["time_shift_variance"].compute_log_likelihood(
                self.fixed_effects["time_shift_variance"]
            )
    ...
    def _compute_class2_priors_regularity(
        self, template_data, control_points, momenta, modulation_matrix
    ):
        """
        Fully torch.
        ...
        if not self.is_frozen["template_data"]:
            for key, value in template_data.items():
                regularity += self.priors["template_data"][
                    key
                ].compute_log_likelihood_torch(value, self.tensor_scalar_type)
        ...
        if not self.is_frozen["momenta"]:
            regularity += self.priors["momenta"].compute_log_likelihood_torch(
                momenta, self.tensor_scalar_type
            )
    ...
    def _compute_residuals(
        self, dataset, template_data, absolute_times, sources, with_grad=True
    ):
        """
        ...
        if self.number_of_processes > 1:
            ...
            for i in range(len(targets)):
                residuals_i = []
                for j, (absolute_time, target) in enumerate(
                    zip(absolute_times[i], targets[i])
                ):
                    ...
                    if with_grad:
                        checkpoint_tensors += list(initial_template_points.values()) + [
                            initial_control_points,
                            initial_momenta,
                        ]

                    tmp_ij.append((i, j))
                    tmp_initial_template_points.append(
                        {
                            key: value.detach()
            ...
            if len(tmp_ij) != 0:
                assert (
                    len(tmp_ij)
                    == len(tmp_initial_template_points)
                    == len(tmp_initial_control_points)
                    == len(tmp_initial_momenta)
                    == len(tmp_targets)
                )
                args.append(
                    (
            ...
            results = self.pool.map(
                compute_exponential_and_attachment, args, chunksize=1
            )
            logger.debug(
                "time taken to compute residuals: "
                + str(time.perf_counter() - start)
                + " for "
                + str(len(args))
                + " tasks with a block_size of "
                + str(block_size)
            )
            ...
            for result in results:
                ...
                if with_grad:
                    for (
                        ...
                        grad_checkpoint_tensors += list(
                            grad_template_point.values()
                        ) + [grad_control_point, grad_momenta]
            ...
            for i in range(len(targets)):
                ...
                for j, (absolute_time, target) in enumerate(
                    ...
                    # target = utilities.convert_deformable_object_to_torch(target, device=device)
                    deformed_points = (
                        self.spatiotemporal_reference_frame.get_template_points(
            ...
            logger.debug(
                "time taken to compute residuals: " + str(time.perf_counter() - start)
            )
    ...
    def _compute_absolute_times(self, times, onset_ages, accelerations):
        ...
        reference_time_torch = torch.from_numpy(np.array(reference_time)).type(
            self.tensor_scalar_type
        )
        ...
        for i in range(len(times)):
            ...
            for j in range(len(times[i])):
                t_ij = torch.from_numpy(np.array(times[i][j])).type(
                    self.tensor_scalar_type
                )

                assert i < len(onset_ages), (
                    "i=" + str(i) + ", len(onset_ages)=" + str(len(onset_ages))
                )

                absolute_times_i.append(
                    clamped_accelerations[i] * (t_ij - onset_ages[i])
                    + reference_time_torch
                )
            absolute_times.append(absolute_times_i)

        tmin = min(
            [
                subject_times[0].detach().cpu().numpy()
        ...
        tmax = max(
            [
                subject_times[-1].detach().cpu().numpy()
                for subject_times in absolute_times
            ]
            + [reference_time]
        )
    ...
    def _fixed_effects_to_torch_tensors(self, with_grad, device="cpu"):
        ...
        modulation_matrix = utilities.move_data(
            modulation_matrix,
            dtype=self.tensor_scalar_type,
    ...
    def _individual_RER_to_torch_tensors(self, individual_RER, with_grad, device="cpu"):
        ...
        # Onset ages.
        onset_ages = individual_RER["onset_age"]
        onset_ages = utilities.move_data(
            onset_ages,
            dtype=self.tensor_scalar_type,
            requires_grad=with_grad,
            device=device,
        ...
        # Accelerations.
        accelerations = individual_RER["acceleration"]
        accelerations = utilities.move_data(
            accelerations,
            dtype=self.tensor_scalar_type,
            requires_grad=with_grad,
            device=device,
    ...
    def adapt_to_error(self, error):
        if (
            error[:64]
            == "Absurd required renormalization factor during parallel transport"
        ):
    ...
    def _augment_discretization(self):
        current_concentration = (
            self.spatiotemporal_reference_frame.get_concentration_of_time_points()
        )
        momenta_factor = current_concentration / float(current_concentration + 1)
        logger.info(
            "Incrementing the concentration of time-points from %d to %d, and multiplying the momenta "
            "by a factor %.3f."
            % (current_concentration, current_concentration + 1, momenta_factor)
        )
        self.spatiotemporal_reference_frame.set_concentration_of_time_points(
    ...
    def print(self, individual_RER):
        ...
        logger.info(
            ">> Spatiotemporal reference frame length: %.2f."
            % (
                self.spatiotemporal_reference_frame.get_tmax()
                - self.spatiotemporal_reference_frame.get_tmin()
            )
        )
    ...
    def write(
        ...
        if write_residuals:
            residuals_list = [
                [
                    [
                        residuals_i_j_k.detach().cpu().numpy()
                        for residuals_i_j_k in residuals_i_j
                    ]
                    for residuals_i_j in residuals_i
                ]
    ...
    def _write_model_predictions(
        self, dataset, individual_RER, output_dir, compute_residuals=True
    ):
        ...
        for i, subject_id in enumerate(dataset.subject_ids):
            residuals_i = []
            for j, (time, absolute_time) in enumerate(
                zip(dataset.times[i], absolute_times[i])
            ):
                deformed_points = (
                    self.spatiotemporal_reference_frame.get_template_points(
                        absolute_time, sources[i]
                ...
                if compute_residuals:
                    residuals_i.append(
                        self.multi_object_attachment.compute_distances(
                            deformed_data, self.template, targets[i][j]
                        )
                    )
    ...
    def _write_model_parameters(self, individual_RER, output_dir):
        ...
        write_2D_array(
            np.zeros((1,)) + math.sqrt(self.get_time_shift_variance()),
            output_dir,
            self.name + "__EstimatedParameters__TimeShiftStd.txt",
        )
        write_2D_array(
            np.zeros((1,)) + math.sqrt(self.get_acceleration_variance()),
            output_dir,
            self.name + "__EstimatedParameters__AccelerationStd.txt",
        )
        write_2D_array(
            np.sqrt(self.get_noise_variance()),
            output_dir,
        ...
        write_2D_array(
            ...
            output_dir,
            self.name + "__EstimatedParameters__Sources.txt",
        )
        # Onset age.
        write_2D_array(
            individual_RER["onset_age"],
            output_dir,
            self.name + "__EstimatedParameters__OnsetAges.txt",
        )
        # Log-acceleration.
        write_2D_array(
            individual_RER["acceleration"],
            output_dir,
