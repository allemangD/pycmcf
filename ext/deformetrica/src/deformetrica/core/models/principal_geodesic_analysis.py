...
class PrincipalGeodesicAnalysis(AbstractStatisticalModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        AbstractStatisticalModel.__init__(
            self, name="PrincipalGeodesicAnalysis", gpu_mode=gpu_mode
        )
        ...
        if self.number_of_processes > 1:
            logger.info(
                "Number of threads larger than 1 not currently handled by the PGA model."
            )
        ...
        self.individual_random_effects["latent_positions"] = (
            MultiScalarNormalDistribution(
                variance=1.0,
            )
        )
        self.individual_random_effects["latent_positions"].set_mean(
            np.zeros(self.latent_space_dimension)
        )
    ...
    def initialize(
        ...
        determ_atlas = deformetrica.estimate_deterministic_atlas(
            template_specifications,
            dataset_specifications,
            model_options,
            determ_estimator_options,
            write_output=True,
        )

        control_points = read_2D_array(
            os.path.join(
                deformetrica.output_dir,
        ...
        latent_positions, components = self._pca_fit_and_transform(
            self.latent_space_dimension, momenta
        )
    ...
    def _pca_fit_and_transform(self, n_components, observations):
        ...
        assert dim >= n_components, (
            "Cannot estimate more components that the dimension of the observations"
        )
        assert n_components <= nb_obs, (
            "Cannot estimate more components than the number of observations"
        )
        ...
        logger.info(
            ">> Total explained variance ratio: %.2f %%"
            % (100.0 * sum(pca.explained_variance_ratio_))
        )
    ...
    def initialize_noise_variance(self, dataset, individual_RER):
        ...
        for k, scalar in enumerate(self.objects_noise_variance_prior_scale_std):
            self.priors["noise_variance"].scale_scalars.append(1.0)

        sufficient_statistics = self.compute_sufficient_statistics(
            dataset, individual_RER, device=device
        )
    ...
    def get_momenta(self):
        principal_directions = torch.from_numpy(self.get_principal_directions()).type(
            self.tensor_scalar_type
        )
        latent_positions = torch.from_numpy(self.get_latent_positions()).type(
            self.tensor_scalar_type
        )
        return self._momenta_from_latent_positions(
            principal_directions, latent_positions
        )
    ...
    def compute_log_likelihood(
        self, dataset, population_RER, individual_RER, mode="complete", with_grad=False
    ):
        """
        ...
        latent_positions = self._individual_RER_to_torch_tensors(
            individual_RER, with_grad, device=device
        )

        momenta = self._momenta_from_latent_positions(
            principal_directions, latent_positions
        )
        ...
        if mode == "complete" and with_grad == False:
            sufficient_statistics = self.compute_sufficient_statistics(
                dataset, individual_RER, residuals=residuals, device=device
            )
        ...
        if mode == "complete":
            regularity += self._compute_random_effects_regularity(
                latent_positions, device=device
            )
        ...
        if with_grad:
            assert regularity.device == attachment.device, (
                "tensors must be on the same device. "
                "regularity.device="
                + str(regularity.device)
                + ", attachment.device="
                + str(attachment.device)
            )
            ...
            if not self.is_frozen["template_data"]:
                if "landmark_points" in template_data.keys():
                    if self.use_sobolev_gradient:
                        ...
                        gradient["landmark_points"] = (
                            template_points["landmark_points"]
                            .grad.detach()
                            .cpu()
                            .numpy()
                        )
            ...
            if not self.is_frozen["principal_directions"]:
                gradient["principal_directions"] = (
                    principal_directions.grad.detach().cpu().numpy()
                )

            if mode == "complete":
                gradient["latent_positions"] = (
                    latent_positions.grad.detach().cpu().numpy()
                )
    ...
    def compute_sufficient_statistics(
        self, dataset, individual_RER, residuals=None, device="cpu"
    ):
        """
        ...
        if residuals is None:
            ...
            # Latent positions
            latent_positions = utilities.move_data(
                individual_RER["latent_positions"],
                dtype=self.tensor_scalar_type,
                device=device,
            )
    ...
    def update_fixed_effects(self, dataset, sufficient_statistics):
        ...
        for k in range(self.number_of_objects):
            noise_variance[k] = (
                sufficient_statistics["S2"][k] + prior_scale_scalars[k] * prior_dofs[k]
            ) / float(
    ...
    def _compute_random_effects_regularity(self, latent_positions, device="cpu"):
        ...
        for i in range(number_of_subjects):
            regularity += self.individual_random_effects[
                ...
                latent_positions[i], self.tensor_scalar_type, device=device
            )
    ...
    def _initialize_latent_positions(self):
        ...
        logger.info(
            "Latent positions initialized to zero, for "
            + str(self.number_of_subjects)
            + " subjects."
        )
    ...
    def _fixed_effects_to_torch_tensors(self, with_grad, device="cpu"):
        ...
        template_data = {
            key: utilities.move_data(
                value, dtype=self.tensor_scalar_type, device=device
            )
        ...
        if self.dense_mode:
            ...
            control_points = utilities.move_data(
                control_points, dtype=self.tensor_scalar_type, device=device
            )
            control_points.requires_grad_(
                (not self.is_frozen["control_points"] and with_grad)
                or self.exponential.get_kernel_type() == "keops"
            )

        pd = self.fixed_effects["principal_directions"]
        principal_directions = utilities.move_data(
            pd, dtype=self.tensor_scalar_type, device=device
        )
        principal_directions.requires_grad_(
            not self.is_frozen["principal_directions"] and with_grad
        )
    ...
    def _momenta_from_latent_positions(self, principal_directions, latent_positions):
        assert latent_positions.device == principal_directions.device, (
            "tensors must be on the same device. "
            "latent_positions.device="
            + str(latent_positions.device)
            + ", principal_directions.device="
            + str(principal_directions.device)
        )

        a, b = self.get_control_points().shape

        return torch.mm(latent_positions, principal_directions).reshape(
    ...
    def write(
        self, dataset, population_RER, individual_RER, output_dir, write_residuals=True
    ):

        # Write the model predictions, and compute the residuals at the same time.
        residuals = self._write_model_predictions(
            dataset, individual_RER, output_dir, compute_residuals=write_residuals
        )

        # Write residuals.
        if write_residuals:
            residuals_list = [
                [residuals_i_k.data.cpu().numpy() for residuals_i_k in residuals_i]
    ...
    def _write_model_predictions(
        ...
        latent_positions = self._individual_RER_to_torch_tensors(
            individual_RER, False, device=device
        )
        ...
        write_2D_array(
            latent_positions.detach().cpu().numpy(),
            output_dir,
    ...
    def _write_principal_directions(self, output_dir):
        ...
        for i in range(self.latent_space_dimension):
            for l, pos in enumerate(np.arange(-1.0, 1.0, 0.2)):
                lp = np.zeros(self.latent_space_dimension)
                lp[i] = 1.0
                lp = pos * lp
                lp_torch = utilities.move_data(
                    lp, dtype=self.tensor_scalar_type, device=device
                )
                momenta = torch.mv(principal_directions.transpose(0, 1), lp_torch).view(
                    control_points.size()
                )
