...
def _subject_attachment_and_regularity(arg):
    ...
    assert torch.device(device) == control_points.device == momenta.device, (
        "control_points and momenta tensors must be on the same device. "
        "device="
        + device
        + ", control_points.device="
        + str(control_points.device)
        + ", momenta.device="
        + str(momenta.device)
    )

    attachment, regularity = (
        DeterministicAtlas._deform_and_compute_attachment_and_regularity(
            exponential,
            template_points,
            control_points,
            momenta,
...
class DeterministicAtlas(AbstractStatisticalModel):
    ...
    def setup_multiprocess_pool(self, dataset):
        self._setup_multiprocess_pool(
            initargs=(
                [target[0] for target in dataset.deformable_objects],
                self.multi_object_attachment,
                self.objects_noise_variance,
                self.freeze_template,
                self.freeze_control_points,
                self.freeze_momenta,
                self.exponential,
                self.sobolev_kernel,
                self.use_sobolev_gradient,
    ...
    def compute_log_likelihood(
        ...
        if self.number_of_processes > 1:
            ...
            results = self.pool.map(
                _subject_attachment_and_regularity, args, chunksize=1
            )  # TODO: optimized chunk size
            ...
            logger.debug(
                "time taken for deformations : " + str(time.perf_counter() - start)
            )
    ...
    @staticmethod
    def _deform_and_compute_attachment_and_regularity(
        exponential,
        template_points,
        control_points,
        momenta,
        ...
        attachment = -multi_object_attachment.compute_weighted_distance(
            deformed_data, template, deformable_objects, objects_noise_variance
        )
        regularity = -exponential.get_norm_squared()

        assert torch.device(device) == attachment.device == regularity.device, (
            "attachment and regularity tensors must be on the same device. "
            "device="
            + device
            + ", attachment.device="
    ...
    def _compute_attachment_and_regularity(
        self,
        dataset,
        template_data,
        template_points,
        ...
        # Compute gradient.
        return self._compute_gradients(
            attachment,
            regularity,
            template_data,
            self.freeze_template,
