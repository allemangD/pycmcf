...
class AffineAtlas(AbstractStatisticalModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.is_frozen = {
            "translation_vectors": freeze_translation_vectors,
            "rotation_angles": freeze_rotation_angles,
            "scaling_ratios": freeze_scaling_ratios,
        }
        ...
        if (
            ...
            template_mean_point = np.mean(
                self.template.get_points()["landmark_points"], axis=0
            )
            for i, target in enumerate(targets):
                target_mean_point = np.mean(
                    target.get_points()["landmark_points"], axis=0
                )
    ...
    def compute_log_likelihood(
        ...
        if with_grad:
            ...
            for i, target in enumerate(targets):
                translation_vector_i = self.tensor_scalar_type(
                    translation_vectors[i]
                ).requires_grad_(
                    with_grad and not self.is_frozen["translation_vectors"]
                )
                rotation_angles_i = self.tensor_scalar_type(
                    rotation_angles[i]
                ).requires_grad_(with_grad and not self.is_frozen["rotation_angles"])
                scaling_ratio_i = self.tensor_scalar_type(
                    [scaling_ratios[i]]
                ).requires_grad_(with_grad and not self.is_frozen["scaling_ratios"])

                deformed_points = self._deform(
                    translation_vector_i,
                    rotation_angles_i,
                    scaling_ratio_i,
                    template_points,
                )
                ...
                if not self.is_frozen["translation_vectors"]:
                    gradient["translation_vectors"][i] = (
                        -translation_vector_i.grad.detach().cpu().numpy()
                    )
                if not self.is_frozen["rotation_angles"]:
                    gradient["rotation_angles"][i] = (
                        -rotation_angles_i.grad.detach().cpu().numpy()
                    )
                if not self.is_frozen["scaling_ratios"]:
                    gradient["scaling_ratios"][i] = (
                        -scaling_ratio_i.grad.detach().cpu().numpy()
                    )
    ...
    def _deform(self, translation_vector, rotation_angles, scaling_ratio, points):
        return self._scale(
            scaling_ratio,
            self._rotate(
                rotation_angles, AffineAtlas._translate(translation_vector, points)
            ),
        )

    def _deform_inverse(
        self, translation_vector, rotation_angles, scaling_ratio, points
    ):
        return self._scale(
            1.0 / scaling_ratio,
            self._rotate(
    ...
    def _scale(scaling_ratio, points):
        ...
        for key, value in points.items():
            if key == "landmark_points":
                ...
                out[key] = (
                    scaling_ratio * (value - center_of_gravity) + center_of_gravity
                )
    ...
    def _rotate(self, rotation_angles, points):
        ...
        for key, value in points.items():
            if key == "landmark_points":
                out[key] = torch.matmul(
                    rotation_matrix.unsqueeze(0), value.unsqueeze(self.dimension - 1)
    ...
    def _compute_rotation_matrix(self, rotation_angles):
        ...
        if self.dimension == 2:
            ...
            self.dimension == 3
        ):  # Using Euler angles: https://fr.wikipedia.org/wiki/Matrice_de_rotation
    ...
    def _initialize_translation_vectors(self):
        ...
        if self.get_translation_vectors() is None:
            ...
            logger.info(
                "Translation vectors initialized to zero, for "
    ...
    def _initialize_rotation_angles(self):
        ...
        if self.get_rotation_angles() is None:
            ...
            logger.info(
                "Rotation Euler angles initialized to zero, for "
                + str(self.number_of_subjects)
                + " subjects."
            )
    ...
    def _initialize_scaling_ratios(self):
        ...
        if self.get_scaling_ratios() is None:
            ...
            logger.info(
                "Scaling ratios initialized to one, for "
                + str(self.number_of_subjects)
                + " subjects."
            )
    ...
    def _write_model_predictions(
        ...
        for i, (subject_id, target) in enumerate(zip(dataset.subject_ids, targets)):
            ...
            target_points = {
                key: self.tensor_scalar_type(value)
                for key, value in target.get_points().items()
            }
            target_data = {
                key: self.tensor_scalar_type(value)
                for key, value in target.get_data().items()
            }

            deformed_points = self._deform_inverse(
                translation_vector_i, rotation_angles_i, scaling_ratio_i, target_points
            )
    ...
    def _write_model_parameters(self, output_dir):
        write_2D_array(
            self.get_translation_vectors(),
            output_dir,
            self.name + "__EstimatedParameters__TranslationVectors.txt",
        )
        write_3D_array(
            self.get_rotation_angles(),
            output_dir,
            self.name + "__EstimatedParameters__RotationAngles.txt",
        )
        write_2D_array(
            self.get_scaling_ratios(),
            output_dir,
            self.name + "__EstimatedParameters__ScalingRatios.txt",
        )
