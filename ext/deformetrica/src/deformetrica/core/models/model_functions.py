...
def initialize_control_points(
    ...
    if initial_control_points is not None:
        ...
        logger.info(
            ">> Reading %d initial control points from file %s."
            % (len(control_points), initial_control_points)
        )

    else:
        if not dense_mode:
            control_points = create_regular_grid_of_points(
                template.bounding_box, spacing, dimension
            )
...
def initialize_momenta(
    ...
    if initial_momenta is not None:
        ...
        if number_of_subjects == 0:
            if random:
                momenta = np.random.randn(
                    number_of_control_points, dimension
                ) / math.sqrt(number_of_control_points * dimension)
            ...
            momenta = np.zeros(
                (number_of_subjects, number_of_control_points, dimension)
            )
            logger.info(
                ">> Momenta initialized to zero, for %d subjects." % number_of_subjects
            )
...
def initialize_covariance_momenta_inverse(control_points, kernel, dimension):
    return np.kron(
        kernel.get_kernel_matrix(torch.from_numpy(control_points)).detach().numpy(),
        np.eye(dimension),
    )


def initialize_modulation_matrix(
    initial_modulation_matrix, number_of_control_points, number_of_sources, dimension
):
    if initial_modulation_matrix is not None:
        ...
        modulation_matrix = np.zeros(
            (number_of_control_points * dimension, number_of_sources)
        )
...
def initialize_accelerations(initial_accelerations, number_of_subjects):
    if initial_accelerations is not None:
        ...
        logger.info(
            ">> Reading initial accelerations from file: " + initial_accelerations
        )
    else:
...
def remove_useless_control_points(control_points, image, kernel_width):
    control_voxels = points_to_voxels_transform(
        control_points, image.affine
    )  # To be modified if image + mesh case.
    ...
    for control_point, control_voxel in zip(control_points, control_voxels):
        ...
        for d in range(image.dimension):
            axe = np.arange(
                max(int(control_voxel[d] - region_size), 0),
        ...
        if (
            image.dimension == 2
            and np.any(
                intensities[neighbouring_voxels[:, 0], neighbouring_voxels[:, 1]]
                > threshold
            )
        ) or (
            image.dimension == 3
            and np.any(
                intensities[
                    neighbouring_voxels[:, 0],
                    neighbouring_voxels[:, 1],
                    neighbouring_voxels[:, 2],
                ]
                > threshold
            )
        ):
