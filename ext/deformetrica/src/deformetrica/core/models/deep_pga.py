...
class DeepPga(AbstractStatisticalModel):
    ...
    def __init__(self):
        ...
        self.individual_random_effects["latent_position"] = (
            MultiScalarNormalDistribution()
        )
    ...
    def compute_log_likelihood(
        ...
        residuals = self._compute_residuals(
            dataset, metric_parameters, latent_positions, with_grad=with_grad
        )
        ...
        if with_grad:
            ...
            if mode in ["complete", "class2"]:
                return attachment.detach().cpu().numpy()[
                    0
                ], regularity.detach().cpu().numpy()[0]
            elif mode == "model":
                return attachments.detach().cpu().numpy()

    def maximize(self, individual_RER, dataset):
        latent_positions = torch.from_numpy(individual_RER["latent_position"]).type(
        ...
        if self.observation_type == "image":
            data = np.array(
                [
                    elt[0].object_list[0].get_intensities()
    ...
    def _fixed_effects_to_torch_tensors(self, with_grad):
        metric_parameters = (
            ...
            .requires_grad_((not self.is_frozen["metric_parameters"]) and with_grad)
        )
    ...
    def _individual_RER_to_torch_tensors(self, individual_RER, with_grad):
        latent_positions = (
            torch.from_numpy(individual_RER["latent_position"])
    ...
    def _compute_residuals(
        self, dataset, metric_parameters, latent_positions, with_grad=False
    ):
        ...
        residuals = torch.from_numpy(np.zeros((number_of_objects,))).type(
            Settings().tensor_scalar_type
        )
        ...
        for i in range(number_of_objects):
            ...
            if self.observation_type == "image":
                target_torch = torch.from_numpy(
                    targets[i][0].object_list[0].get_intensities()
                )
        ...
        np.savetxt(
            os.path.join(Settings().output_dir, "residuals.txt"),
            np.array([elt.data.numpy() for elt in residuals]),
        )
    ...
    def _compute_random_effects_regularity(self, latent_positions):
        ...
        for i in range(number_of_objects):
            regularity += self.individual_random_effects[
                "latent_position"
            ].compute_log_likelihood_torch(latent_positions[i])

        # Noise random effect
        regularity -= (
            0.5 * number_of_objects * math.log(self.fixed_effects["noise_variance"])
        )
    ...
    def compute_sufficient_statistics(
        ...
        if residuals is None:
            ...
            residuals = self._compute_residuals(
                dataset, metric_parameters, latent_positions, with_grad=False
            )
        ...
        if not self.is_frozen["noise_variance"]:
            ...
            for i in range(len(residuals)):
                sufficient_statistics["S1"] += (
                    torch.sum(residuals[i]).cpu().detach().numpy()
                )
        ...
        if not self.is_frozen["noise_variance"]:
            ...
            if self.observation_type == "numpy":
                ...
                if Settings().dimension == 2:
                    ...
                    logger.info(
                        "Noise dimension automatically set to 64x64x64 ! watchout"
                    )
    ...
    def _write_model_parameters(self):
        np.savetxt(
            os.path.join(Settings().output_dir, self.name + "_metric_parameters.txt"),
            self.get_metric_parameters(),
        )

    def _write_lsd_coordinates(self, individual_RER):
        np.savetxt(
            os.path.join(Settings().output_dir, self.name + "_latent_position.txt"),
    ...
    def write_sources(self):
        if self.observation_type == "image":
            ...
            for i in range(self.latent_space_dimension):
                ...
                direction[i] = 1.0
                times = np.linspace(-1.0, 1.0, 10)
                for t in times:
                    latent_position = direction * t
                    l_p_torch = torch.from_numpy(latent_position).type(
                        Settings().tensor_scalar_type
                    )
                    img = self.net(l_p_torch)
                    self.template.write(
                        ["sources_" + str(i) + "_" + str(t) + ".png"],
                        {"image_intensities": img.detach().numpy()},
                    )
    ...
    def _write_model_predictions(self, dataset, individual_RER, sample=False):
        ...
        targets = dataset.deformable_objects  # A list of list
        latent_positions = self._individual_RER_to_torch_tensors(
            individual_RER, with_grad=False
        )
        metric_parameters = self._fixed_effects_to_torch_tensors(with_grad=False)
        ...
        if self.observation_type == "image":
            for i in range(number_of_objects):
                ...
                self.template.write(
                    ["reconstructed_" + str(i) + ".png"],
                    {"image_intensities": prediction.detach().numpy()},
                )

        elif self.observation_type == "numpy":
            if self.number_iterations == 0:
                observations = np.array(dataset.deformable_objects).reshape(
                    dataset.number_of_subjects, Settings().dimension
                )
                np.save(
                    os.path.join(Settings().output_dir, "observations.npy"),
                    observations,
                )
            ...
            np.save(
                os.path.join(
                    Settings().output_dir,
                    "reconstructed_observations_"
                    + str(self.number_iterations)
                    + ".npy",
                ),
