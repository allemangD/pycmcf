...
class LongitudinalMetricLearning(AbstractStatisticalModel):
    ...
    def __init__(self):
        ...
        self.observation_type = (
            None  # image of scalar, to compute the distances efficiently in here.
        )
        ...
        self.priors["log_acceleration_variance"] = (
            MultiScalarInverseWishartDistribution()
        )
        ...
        self.individual_random_effects["log_acceleration"] = (
            MultiScalarNormalDistribution()
        )
    ...
    def set_metric_parameters(self, metric_parameters):
        ...
        if self.deep_metric_learning:
            ...
            self.fixed_effects["metric_parameters"] = (
                self.spatiotemporal_reference_frame.project_metric_parameters(
                    metric_parameters
                )
            )
    ...
    def get_fixed_effects(self):
        ...
        if not self.is_frozen["metric_parameters"] and (
            self.parametric_metric or self.deep_metric_learning
        ):
    ...
    def compute_log_likelihood(
        ...
        :return:
        """
        v0, p0, metric_parameters, modulation_matrix = (
            self._fixed_effects_to_torch_tensors(with_grad)
        )
        onset_ages, log_accelerations, sources = self._individual_RER_to_torch_tensors(
            individual_RER, with_grad
        )
        ...
        if mode == "complete":
            sufficient_statistics = self.compute_sufficient_statistics(
                dataset, population_RER, individual_RER, residuals
            )
        ...
        regularity = self._compute_random_effects_regularity(
        ...
        if with_grad:
            ...
            if not self.is_frozen["metric_parameters"]:
                if self.deep_metric_learning:
                    ...
                    gradient["metric_parameters"] = (
                        self.spatiotemporal_reference_frame.project_metric_parameters_gradient(
                            metric_parameters.cpu().data.numpy(),
                            metric_parameters.grad.data.cpu().numpy(),
                        )
                    )

            if (
                not self.is_frozen["modulation_matrix"]
                and not self.no_parallel_transport
            ):
                gradient["modulation_matrix"] = (
                    modulation_matrix.grad.data.cpu().numpy()
                )
            ...
            if mode in ["complete", "class2"]:
                return (
                    attachment.data.cpu().numpy(),
                    regularity.data.cpu().numpy(),
                    gradient,
                )
            elif mode == "model":
    ...
    def maximize(self, individual_RER, dataset):
        assert self.deep_metric_learning, (
            "Model maximization procedure only defined for deep metric learning"
        )
        ...
        for i, elt in enumerate(dataset.times):
            for j, t in enumerate(elt):
                if self.observation_type == "scalar":
                    ...
                    observations.append(
                        dataset.deformable_objects[i][j].get_intensities()
                    )
        ...
        logger.info(
            "tmin",
            self.spatiotemporal_reference_frame.geodesic.tmin,
            "tmax",
            self.spatiotemporal_reference_frame.geodesic.tmax,
        )

        # plt.savefig(os.path.join(Settings().output_dir, "Latent_space_coordinates.pdf"))
        plt.clf()

        lsd_observations = torch.from_numpy(lsd_observations).type(
            Settings().tensor_scalar_type
        )
        observations = torch.from_numpy(observations).type(
            Settings().tensor_scalar_type
        )
    ...
    def _fixed_effects_to_torch_tensors(self, with_grad):
        ...
        if self.parametric_metric or self.deep_metric_learning:
            metric_parameters = Variable(
                torch.from_numpy(self.fixed_effects["metric_parameters"]),
        ...
        if not self.no_parallel_transport:
            modulation_matrix = Variable(
                torch.from_numpy(self.get_modulation_matrix()),
    ...
    def _individual_RER_to_torch_tensors(self, individual_RER, with_grad):
        onset_ages = Variable(
            torch.from_numpy(individual_RER["onset_age"]).type(
                Settings().tensor_scalar_type
            ),
            requires_grad=with_grad,
        )
        log_accelerations = Variable(
            torch.from_numpy(individual_RER["log_acceleration"]).type(
                Settings().tensor_scalar_type
            ),
            requires_grad=with_grad,
        )
        ...
        if not self.no_parallel_transport:
            sources = Variable(
                torch.from_numpy(individual_RER["sources"]).type(
                    Settings().tensor_scalar_type
                ),
                requires_grad=with_grad,
            )
    ...
    def _compute_residuals(
        ...
        absolute_times = self._compute_absolute_times(
            dataset.times, log_accelerations, onset_ages
        )

        self._update_spatiotemporal_reference_frame(
            absolute_times, p0, v0, metric_parameters, modulation_matrix
        )
        ...
        for i in range(number_of_subjects):
            if self.observation_type == "image":
                targets_torch = Variable(
                    torch.from_numpy(
                        np.array([e.get_intensities() for e in targets[i]])
                    ).type(Settings().tensor_scalar_type)
                )
            else:
            ...
            if not self.deep_metric_learning:
                for j, t in enumerate(absolute_times[i]):
                    if sources is not None:
                        predicted_values_i[j] = (
                            self.spatiotemporal_reference_frame.get_position(
                                t, sources=sources[i]
                            )
                        )
                    else:
                        predicted_values_i[j] = (
                            self.spatiotemporal_reference_frame.get_position(t)
                        )
            ...
            if Settings().dimension == 1 or self.observation_type == "scalar":
                ...
                residuals.append(
                    torch.sum(torch.sum(residuals_i.view(targets_torch.size()), 1), 1)
                )
            else:
                ...
                residuals.append(
                    torch.sum(
                        torch.sum(
                            torch.sum(residuals_i.view(targets_torch.size()), 1), 1, 1
                        )
                    )
                )
    ...
    def _compute_individual_attachments(self, residuals):
        ...
        attachments = Variable(
            torch.zeros((number_of_subjects,)).type(Settings().tensor_scalar_type),
            requires_grad=False,
        )
        noise_variance_torch = Variable(
            torch.from_numpy(np.array([self.fixed_effects["noise_variance"]])).type(
                Settings().tensor_scalar_type
            ),
            requires_grad=False,
        )
    ...
    def _compute_absolute_times(self, times, log_accelerations, onset_ages):
        ...
        if (
            np.max(accelerations.cpu().data.numpy()) > upper_threshold
            or np.min(accelerations.cpu().data.numpy()) < lower_threshold
        ):
            raise ValueError(
                "Absurd numerical value for the acceleration factor. Exception raised."
            )

        absolute_times = []
        for i in range(len(times)):
            absolute_times_i = (
                Variable(torch.from_numpy(times[i]).type(Settings().tensor_scalar_type))
                - onset_ages[i]
            ) * accelerations[i] + reference_time
    ...
    def _compute_absolute_time(self, time, acceleration, onset_age, reference_time):
        return (
            Variable(
                torch.from_numpy(np.array([time])).type(Settings().tensor_scalar_type)
            )
            - onset_age
        ) * acceleration + reference_time

    ####################################################################################################################
    ### Private methods:
    ####################################################################################################################

    def _compute_random_effects_regularity(
        self, log_accelerations, onset_ages, sources
    ):
        """
        Fully torch.
        """
        ...
        for i in range(number_of_subjects):
            regularity += self.individual_random_effects[
                "onset_age"
            ].compute_log_likelihood_torch(onset_ages[i])

        # Log-acceleration random effect.
        for i in range(number_of_subjects):
            regularity += self.individual_random_effects[
                "log_acceleration"
            ].compute_log_likelihood_torch(log_accelerations[i])

        if sources is not None:
            for i in range(number_of_subjects):
                regularity += self.individual_random_effects[
                    "sources"
                ].compute_log_likelihood_torch(sources[i])

        # Noise random effect
        regularity -= (
            0.5 * number_of_subjects * math.log(self.fixed_effects["noise_variance"])
        )

        return regularity

    def compute_sufficient_statistics(
        self, dataset, population_RER, individual_RER, residuals=None, model_terms=None
    ):
        ...
        if residuals is None:
            ...
            onset_ages, log_accelerations, sources = (
                self._individual_RER_to_torch_tensors(individual_RER, False)
            )
        ...
        if not self.is_frozen["noise_variance"]:
            sufficient_statistics["S1"] = 0.0
            for i in range(len(residuals)):
                sufficient_statistics["S1"] += (
                    torch.sum(residuals[i]).cpu().data.numpy()[0]
                )
        ...
        if (
            not self.is_frozen["reference_time"]
            or not self.is_frozen["onset_age_variance"]
        ):
        ...
        if len(log_accelerations) > 1:
            sufficient_statistics["S5"] = np.exp(
                np.mean(log_accelerations.data.numpy())
            )
    ...
    def update_fixed_effects(self, dataset, sufficient_statistics):
        ...
        if not self.is_frozen["log_acceleration_variance"]:
            ...
            log_acceleration_variance = (
                sufficient_statistics["S2"] + prior_dof * prior_scale
        ...
        if not self.is_frozen["onset_age_variance"]:
            ...
            onset_age_prior_dof = self.priors["onset_age_variance"].degrees_of_freedom[
                0
            ]
            onset_age_variance = (
                sufficient_statistics["S4"]
                + onset_age_prior_dof * onset_age_prior_scale
    ...
    def _compute_class1_priors_regularity(self):
        ...
        if not self.is_frozen["onset_age_variance"]:
            regularity += self.priors["onset_age_variance"].compute_log_likelihood(
                self.fixed_effects["onset_age_variance"]
            )

        # Log-acceleration variance prior
        if not self.is_frozen["log_acceleration_variance"]:
            regularity += self.priors[
                "log_acceleration_variance"
            ].compute_log_likelihood(self.fixed_effects["log_acceleration_variance"])
    ...
    def _update_spatiotemporal_reference_frame(
        self, absolute_times, p0, v0, metric_parameters, modulation_matrix
    ):
        ...
        tmin = min(
            [subject_times[0].cpu().data.numpy()[0] for subject_times in absolute_times]
            + [t0]
        )
        tmax = max(
            [
                subject_times[-1].cpu().data.numpy()[0]
        ...
        if modulation_matrix is not None:
            self.spatiotemporal_reference_frame.set_modulation_matrix_t0(
                modulation_matrix
            )
    ...
    def _get_lsd_observations(self, individual_RER, dataset):
        ...
        for i, elt in enumerate(dataset.times):
            for j, t in enumerate(elt):
                ...
                lsd_obs = (
                    (abs_time - reference_time) * v0
                    + p0
                    + np.matmul(modulation_matrix, sources[i])
                )
    ...
    def initialize_onset_age_variables(self):
        ...
        if self.individual_random_effects["onset_age"].mean is None:
            raise RuntimeError(
                "The set_reference_time method of a LongitudinalAtlas model should be called before "
                "the update one."
            )
        ...
        if self.individual_random_effects["onset_age"].variance_sqrt is None:
            raise RuntimeError(
                "The set_time_shift_variance method of a LongitudinalAtlas model should be called "
                "before the update one."
            )

        if not self.is_frozen["onset_age_variance"]:
            # Set the time_shift_variance prior scale to the initial time_shift_variance fixed effect.
            self.priors["onset_age_variance"].scale_scalars.append(
                self.get_onset_age_variance()
            )
            logger.info(
                ">> The time shift variance prior degrees of freedom parameter is set to",
                self.number_of_subjects,
            )
            self.priors["onset_age_variance"].degrees_of_freedom.append(
                self.number_of_subjects
            )
    ...
    def initialize_log_acceleration_variables(self):
        ...
        if not self.is_frozen["log_acceleration_variance"]:
            ...
            self.priors["log_acceleration_variance"].scale_scalars.append(
                self.get_log_acceleration_variance()
            )
            # Arbitrarily set the log_acceleration_variance prior dof to 1.
            logger.info(
                ">> The log-acceleration variance prior degrees of "
                "freedom parameter is set to the number of subjects:",
                self.number_of_subjects,
            )
            self.priors["log_acceleration_variance"].degrees_of_freedom.append(
                self.number_of_subjects
            )
    ...
    def initialize_source_variables(self):
        ...
        if self.number_of_sources is None:
            raise RuntimeError(
                "The number of sources must be set before calling the update method "
        ...
        if self.no_parallel_transport:
            ...
            self.individual_random_effects["sources"].mean = np.zeros(
    ...
    def initialize_modulation_matrix_variables(self):
        ...
        if self.no_parallel_transport:
            ...
            if self.fixed_effects["modulation_matrix"] is None:
                ...
                self.fixed_effects["modulation_matrix"] = np.zeros(
                    (len(self.get_p0()), self.number_of_sources)
                )

            else:
                assert (
                    self.number_of_sources == self.get_modulation_matrix().shape[1]
                ), "The number of sources should be set somewhere"
        ...
        if not self.is_frozen["modulation_matrix"]:
            ...
            self.priors["modulation_matrix"].set_variance_sqrt(1.0)
    ...
    def initialize_deep_metric_learning(self):
        ...
        assert self.spatiotemporal_reference_frame.geodesic.manifold_type == "deep", (
            "Oups"
        )
        ...
        if self.observation_type == "scalar":
            self.net = ScalarNet(
                in_dimension=self.latent_space_dimension,
                out_dimension=Settings().dimension,
            )
    ...
    def _write_model_parameters(self):
        ...
        if not self.no_parallel_transport:
            write_2D_array(
                self.get_modulation_matrix(), self.name + "_modulation_matrix.txt"
            )
        ...
        np.save(
            os.path.join(Settings().output_dir, self.name + "_all_fixed_effects.npy"),
            self.fixed_effects,
        )
    ...
    def _write_individual_RER(self, dataset, individual_RER):
        ...
        write_2D_array(
            np.exp(individual_RER["log_acceleration"]), self.name + "_alphas.txt"
        )
        write_2D_array(
            individual_RER["log_acceleration"], self.name + "_log_accelerations.txt"
        )
        write_2D_array(
            np.array(dataset.subject_ids),
            self.name + "_subject_ids_unique.txt",
            fmt="%s",
        )
        if not self.no_parallel_transport:
    ...
    def _write_geodesic_and_parallel_trajectories(self):
        ...
        times_geodesic = np.array(
            self.spatiotemporal_reference_frame.geodesic.get_times()
        )

        geodesic_values = (
            self.spatiotemporal_reference_frame.geodesic.get_geodesic_trajectory()
        )
        ...
        if self.observation_type == "scalar":
            ...
            self._clean_and_create_directory(
                os.path.join(Settings().output_dir, "geodesic_trajectory")
            )
            self._write_image_trajectory(
                range(len(times_geodesic)),
                geodesic_values,
                os.path.join(Settings().output_dir, "geodesic_trajectory"),
                "geodesic",
            )
        ...
        if self.observation_type == "image":
            times_parallel_curves = np.linspace(
                np.min(times_geodesic) + 1e-5, np.max(times_geodesic) - 1e-5, 20
            )

        else:
            times_parallel_curves = np.linspace(
                np.min(times_geodesic) + 1e-5, np.max(times_geodesic) - 1e-5, 200
            )

        # Saving a txt file with the trajectory.
        write_2D_array(
            times_geodesic, self.name + "_reference_geodesic_trajectory_times.txt"
        )
        if self.observation_type == "scalar":
            write_2D_array(
                geodesic_values, self.name + "_reference_geodesic_trajectory_values.txt"
            )
        ...
        if not self.no_parallel_transport and self.number_of_sources > 0:
            if self.parametric_metric:
                write_2D_array(
                    self.spatiotemporal_reference_frame.geodesic.forward_exponential.interpolation_points_torch.cpu().data.numpy(),
                    self.name + "_interpolation_points.txt",
                )

            times_torch = Variable(
                torch.from_numpy(times_parallel_curves).type(
                    Settings().tensor_scalar_type
                )
            )
            ...
            for i in range(self.number_of_sources):
                ...
                source_neg[i] = -1.0
                source_pos_torch = Variable(
                    torch.from_numpy(source_pos).type(Settings().tensor_scalar_type)
                )
                source_neg_torch = Variable(
                    torch.from_numpy(source_neg).type(Settings().tensor_scalar_type)
                )

                trajectory_pos = [
                    self.spatiotemporal_reference_frame.get_position(
                        t, sources=source_pos_torch
                    )
                    for t in times_torch
                ]
                trajectory_neg = [
                    self.spatiotemporal_reference_frame.get_position(
                        t, sources=source_neg_torch
                    )
                    for t in times_torch
                ]
                ...
                trajectory_pos = np.array(
                    [elt.cpu().data.numpy() for elt in trajectory_pos]
                )
                trajectory_neg = np.array(
                    [elt.cpu().data.numpy() for elt in trajectory_neg]
                )

                if self.observation_type == "scalar":
                    write_2D_array(
                        trajectory_pos, self.name + "_source_" + str(i) + "_pos.txt"
                    )
                    write_2D_array(
                        trajectory_neg, self.name + "_source_" + str(i) + "_neg.txt"
                    )
                    write_2D_array(
                        times_parallel_curves, self.name + "_times_parallel_curves.txt"
                    )
                    self._plot_scalar_trajectory(times_geodesic, geodesic_values)
                    self._plot_scalar_trajectory(
                        times_parallel_curves,
                        trajectory_pos,
                    ...
                    self._clean_and_create_directory(
                        os.path.join(
                            Settings().output_dir, "parallel_curve_pos_" + str(i)
                        )
                    )
                    self._clean_and_create_directory(
                        os.path.join(
                            Settings().output_dir, "parallel_curve_neg_" + str(i)
                        )
                    )
                    self._write_image_trajectory(
                        range(len(times_parallel_curves)),
                        trajectory_pos,
                        os.path.join(
                            Settings().output_dir, "parallel_curve_pos_" + str(i)
                        ),
    ...
    def _write_model_predictions(self, dataset, individual_RER, sample=False):
        ...
        np.savetxt(
            os.path.join(Settings().output_dir, self.name + "_absolute_times.txt"),
        ...
        for i in range(number_of_subjects):
            ...
            for j, t in enumerate(absolute_times[i]):
                if sources is not None:
                    prediction = self.spatiotemporal_reference_frame.get_position(
                        t, sources=sources[i]
                    )
                else:
            ...
            if sample:
                ...
                residuals.append(
                    np.sum(np.linalg.norm(predictions_i - targets_i, axis=0))
                )
                ...
                residuals.append(
                    np.mean(np.linalg.norm(predictions_i - targets_i, axis=0))
                )

            # Now plotting the real data.
            if nb_plot_to_make > 0:
                # We also make a plot of the trajectory and save it.
                times_subject = dataset.times[
                    i
                ]  # np.linspace(dataset.times[i][0], dataset.times[i][-1], 10)
                absolute_times_subject = [
                    self._compute_absolute_time(t, accelerations[i], onset_ages[i], t0)
                    for t in times_subject
                ]

                if sources is None:
                    trajectory = [
                        self.spatiotemporal_reference_frame.get_position(t)
                        for t in absolute_times_subject
                    ]
                else:
                    trajectory = [
                        self.spatiotemporal_reference_frame.get_position(
                ...
                if self.observation_type == "scalar":
                    ...
                    self._plot_scalar_trajectory(
                        times_subject,
                        trajectory,
                        names=["subject " + str(dataset.subject_ids[i])],
                        linestyles=linestyles,
                    )
                    self._plot_scalar_trajectory(
                        [t for t in dataset.times[i]],
                        targets_i,
                        linestyles=linestyles,
                        linewidth=0.2,
                    )
                    ...
                    if Settings().dimension == 2:
                        if len(trajectory.shape) < 3:
                            ...
                            trajectory = trajectory.reshape(
                                len(trajectory), image_dimension, image_dimension
                            )
                    elif Settings().dimension == 3:
        ...
        if sample:
            ...
            if self.observation_type == "scalar":
                write_2D_array(
                    np.array(predictions), self.name + "_reconstructed_values.txt"
                )
    ...
    def _write_image_trajectory(self, times, images, folder, name):
        ...
        if Settings().dimension == 2:
            for j, t in enumerate(times):
                ...
                self.template.write(
                    os.path.join(
                        folder, self.name + "_" + name + "_t__" + str(t) + ".npy"
                    )
                )
        elif Settings().dimension == 3:
            for j, t in enumerate(times):
                ...
                self.template.write(
                    os.path.join(
                        folder, self.name + "_" + name + "_t__" + str(t) + ".nii"
                    )
                )
        else:
