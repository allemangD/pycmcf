...
class SrwMhwgSampler:
    ...
    def sample(
        ...
        if current_model_terms is None:
            current_model_terms = self._compute_model_log_likelihood(
                statistical_model, dataset, population_RER, individual_RER
            )
        ...
        for (
            ...
            for i in range(dataset.number_of_subjects):
                ...
                current_regularity_terms.append(
                    model_RED.compute_log_likelihood(
                ...
                individual_RER[random_effect_name][i] = candidate_RER[i].reshape(
                    shape_parameters
                )
                candidate_regularity_terms.append(
                    model_RED.compute_log_likelihood(candidate_RER[i])
                )
            ...
            candidate_model_terms = self._compute_model_log_likelihood(
                ...
                population_RER,
                individual_RER,
                modified_individual_RER=random_effect_name,
            )
            ...
            for i in range(dataset.number_of_subjects):
                ...
                # Acceptance rate.
                tau = (
                    candidate_model_terms[i]
                    + candidate_regularity_terms[i]
                    - current_model_terms[i]
                    - current_regularity_terms[i]
                )
                ...
                if math.log(np.random.uniform()) > tau or math.isnan(tau):
                    individual_RER[random_effect_name][i] = current_RER[i].reshape(
                        shape_parameters
                    )
            ...
            acceptance_rates[random_effect_name] *= 100.0 / float(
    ...
    def _compute_model_log_likelihood(
        self,
        statistical_model,
        dataset,
        population_RER,
    ...
    def adapt_proposal_distributions(
        self, current_acceptance_rates_in_window, iteration_number, verbose
    ):
        ...
        for (
            ...
            if ar > self.acceptance_rates_target:
                std *= 1 + (ar - goal) / (
                    (100 - goal) * math.sqrt(iteration_number + 1)
                )
            else:
