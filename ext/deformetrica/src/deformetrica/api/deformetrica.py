...
from ..launch.estimate_longitudinal_registration import (
...
class Deformetrica:
    ...
    def __init__(self, output_dir=default.output_dir, verbosity="INFO"):
        ...
        logger_file_handler = logging.FileHandler(
            os.path.join(
                self.output_dir,
                time.strftime("%Y-%m-%d-%H%M%S", time.gmtime()) + "_info.log",
            ),
            mode="w",
        )
        ...
        logger.error(
            "Logger has been set to: "
            + logging.getLevelName(logger_stream_handler.level)
        )
    ...
    def estimate_registration(
        ...
        # Instantiate model.
        statistical_model = DeterministicAtlas(
        ...
        estimator = self.__instantiate_estimator(
            statistical_model, dataset, estimator_options, default=ScipyOptimize
        )
    ...
    def estimate_bayesian_atlas(
        ...
        individual_RER = statistical_model.initialize_random_effects_realization(
            dataset.number_of_subjects, **model_options
        )
    ...
    def estimate_longitudinal_atlas(
        self,
        template_specifications,
        dataset_specifications,
        model_options={},
        ...
        estimator_options["individual_RER"] = individual_RER
        estimator = self.__instantiate_estimator(
            statistical_model, dataset, estimator_options, default=McmcSaem
        )
    ...
    def estimate_longitudinal_registration(
        self,
        template_specifications,
        dataset_specifications,
        model_options={},
    ...
    def estimate_geodesic_regression(
        ...
        assert dataset.is_time_series(), (
            "Cannot estimate a geodesic regression from a non-time-series dataset."
        )
    ...
    def estimate_principal_geodesic_analysis(
        self,
        template_specifications,
        dataset_specifications,
        model_options={},
    ...
    def __instantiate_estimator(
        self, statistical_model, dataset, estimator_options, default=ScipyOptimize
    ):
        if (
            estimator_options["optimization_method_type"].lower()
            == "GradientAscent".lower()
        ):
        ...
        return estimator(
            statistical_model, dataset, output_dir=self.output_dir, **estimator_options
        )
    ...
    def further_initialization(
        ...
        if estimator_options is not None:
            ...
            if (
                estimator_options["gpu_mode"] is GpuMode.FULL
        ...
        if "initial_time_shift_variance" not in model_options:
            model_options["initial_time_shift_variance"] = (
                default.initial_time_shift_variance
            )
        if "initial_modulation_matrix" not in model_options:
            model_options["initial_modulation_matrix"] = (
                default.initial_modulation_matrix
            )
        ...
        if "initial_acceleration_variance" not in model_options:
            model_options["initial_acceleration_variance"] = (
                default.initial_acceleration_variance
            )
        ...
        if "sobolev_kernel_width_ratio" not in model_options:
            model_options["sobolev_kernel_width_ratio"] = (
                default.sobolev_kernel_width_ratio
            )
        ...
        if (
            model_options["initial_cp_spacing"] is None
            and model_options["initial_control_points"] is None
            ...
            model_options["initial_cp_spacing"] = model_options[
        ...
        if "OMP_NUM_THREADS" not in os.environ:
            ...
            omp_num_threads = math.floor(
                os.cpu_count() / model_options["number_of_processes"]
            )
            ...
            logger.info(
                "OMP_NUM_THREADS found in environment variables. Using value OMP_NUM_THREADS="
                + str(os.environ["OMP_NUM_THREADS"])
            )
        ...
        if model_type.lower() in [
            ...
            mean_visit_age = 0.0
            var_visit_age = 0.0
            assert "visit_ages" in dataset_specifications, (
                "Visit ages are needed to estimate a Regression, "
            ...
            if total_number_of_visits > 0:
                ...
                var_visit_age = (
                    var_visit_age / float(total_number_of_visits) - mean_visit_age**2
                )
        ...
        try:
            ...
            logger.warning(
                "Could not set max open file. Currently using: " + str(rlimit)
            )
        ...
        if estimator_options is not None:
            ...
            if (
                ...
                and not model_options["freeze_template"]
                and model_options["use_sobolev_gradient"]
            ):
                logger.info(
        ...
        if (
            model_type.lower() == "LongitudinalAtlas".lower()
            and model_options["initial_modulation_matrix"] is None
            and model_options["number_of_sources"] is None
        ):
            model_options["number_of_sources"] = 4
            logger.info(
                ">> No initial modulation matrix given, neither a number of sources. "
                "The latter will be ARBITRARILY defaulted to %d."
                % model_options["number_of_sources"]
            )

        # Initialize the initial_acceleration_variance if needed.
        if (
            model_type == "LongitudinalAtlas".lower()
            or model_type == "LongitudinalRegistration".lower()
            ...
            if elt["deformable_object_type"].lower() == "image":
                ...
                if not model_options["downsampling_factor"] == 1:
                    if "downsampling_factor" in elt.keys():
                        ...
                        elt["downsampling_factor"] = model_options[
                            "downsampling_factor"
                        ]
                        logger.info(
                            ">> Setting the image grid downsampling factor to: %d."
        ...
        if (
            estimator_options is not None
            and estimator_options["optimization_method_type"].lower()
            == "McmcSaem".lower()
        ):
            assert model_type.lower() in [
                "LongitudinalAtlas".lower(),
            ...
            if model_type.lower() == "LongitudinalAtlas".lower():
                if "onset_age_proposal_std" not in estimator_options:
                    estimator_options["onset_age_proposal_std"] = (
                        default.onset_age_proposal_std
                    )
                if "acceleration_proposal_std" not in estimator_options:
                    estimator_options["acceleration_proposal_std"] = (
                        default.acceleration_proposal_std
                    )
                if "sources_proposal_std" not in estimator_options:
                    estimator_options["sources_proposal_std"] = (
                        default.sources_proposal_std
                    )

                estimator_options["individual_proposal_distributions"] = {
                    "onset_age": MultiScalarNormalDistribution(
                        std=estimator_options["onset_age_proposal_std"]
                    ),
                    "acceleration": MultiScalarNormalDistribution(
                        std=estimator_options["acceleration_proposal_std"]
                    ),
                    "sources": MultiScalarNormalDistribution(
                        std=estimator_options["sources_proposal_std"]
                    ),
                }

            elif model_type.lower() == "BayesianAtlas".lower():
                if "momenta_proposal_std" not in estimator_options:
                    estimator_options["momenta_proposal_std"] = (
                        default.momenta_proposal_std
                    )

                estimator_options["individual_proposal_distributions"] = {
                    "momenta": MultiScalarNormalDistribution(
                        std=estimator_options["momenta_proposal_std"]
                    )
                }
