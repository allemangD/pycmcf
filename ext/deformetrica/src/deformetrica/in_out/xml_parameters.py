...
def get_estimator_options(xml_parameters):
    ...
    if xml_parameters.optimization_method_type.lower() == "GradientAscent".lower():
        ...
        options["max_line_search_iterations"] = (
            xml_parameters.max_line_search_iterations
        )
    ...
    # common options
    options["optimization_method_type"] = (
        xml_parameters.optimization_method_type.lower()
    )
...
def get_model_options(xml_parameters):
    options = {
        ...
        "tensor_scalar_type": utilities.get_torch_scalar_type(
            dtype=xml_parameters.dtype
        ),
        "tensor_integer_type": utilities.get_torch_integer_type(
            dtype=xml_parameters.dtype
        ),
        "random_seed": xml_parameters.random_seed,
    }

    if xml_parameters.model_type.lower() in [
        "LongitudinalAtlas".lower(),
        ...
        options["initial_time_shift_variance"] = (
            xml_parameters.initial_time_shift_variance
        )
        options["initial_acceleration_mean"] = xml_parameters.initial_acceleration_mean
        options["initial_acceleration_variance"] = (
            xml_parameters.initial_acceleration_variance
        )
        ...
        options["freeze_time_shift_variance"] = (
            xml_parameters.freeze_time_shift_variance
        )
        options["freeze_acceleration_variance"] = (
            xml_parameters.freeze_acceleration_variance
        )
        ...
        options["initial_principal_directions"] = (
            xml_parameters.initial_principal_directions
        )
        options["freeze_principal_directions"] = (
            xml_parameters.freeze_principal_directions
        )
        ...
        options["tmin"] = xml_parameters.tmin
        options["tmax"] = xml_parameters.tmax
        options["initial_momenta_to_transport"] = (
            xml_parameters.initial_momenta_to_transport
        )
        options["initial_control_points_to_transport"] = (
...
class XmlParameters:
    ...
    def __init__(self):
        ...
        self.covariance_momenta_prior_normalized_dof = (
            default.covariance_momenta_prior_normalized_dof
        )
        ...
        self._cuda_is_used = (
            default._cuda_is_used
        )  # true if at least one operation will use CUDA.
        self._keops_is_used = (
            default._keops_is_used
        )  # true if at least one keops kernel operation will take place.
        ...
        self.initial_control_points_to_transport = (
            default.initial_control_points_to_transport
        )
        ...
        self.number_of_metric_parameters = (
            default.number_of_metric_parameters
        )  # number of parameters in metric learning.
        self.number_of_interpolation_points = default.number_of_interpolation_points
        self.latent_space_dimension = (
            default.latent_space_dimension
    ...
    def read_all_xmls(
        self, model_xml_path, dataset_xml_path, optimization_parameters_xml_path
    ):
    ...
    def _read_model_xml(self, model_xml_path):
        ...
        for model_xml_level1 in model_xml_level0:
            if model_xml_level1.tag.lower() == "model-type":
                ...
                self.tensor_scalar_type = utilities.get_torch_scalar_type(
                    dtype=self.dtype
                )
                self.tensor_integer_type = utilities.get_torch_integer_type(
                    dtype=self.dtype
                )
                ...
                for model_xml_level2 in model_xml_level1:
                    if model_xml_level2.tag.lower() == "dense-mode":
                        ...
                        template_object = (
                            self._initialize_template_object_xml_parameters()
                        )
                        for model_xml_level3 in model_xml_level2:
                            if model_xml_level3.tag.lower() == "deformable-object-type":
                                template_object["deformable_object_type"] = (
                                    model_xml_level3.text.lower()
                                )
                            elif model_xml_level3.tag.lower() == "attachment-type":
                                template_object["attachment_type"] = (
                                    model_xml_level3.text.lower()
                                )
                                ...
                                model_xml_level3.tag.lower()
                                == "noise-variance-prior-scale-std"
                            ):
                                template_object["noise_variance_prior_scale_std"] = (
                                    float(model_xml_level3.text)
                                )
                            elif (
                                model_xml_level3.tag.lower()
                                ...
                                template_object[
                                    "noise_variance_prior_normalized_dof"
                                ] = float(model_xml_level3.text)
                            else:
                                msg = (
                                    "Unknown entry while parsing the template > "
                                    + model_xml_level2.attrib["id"]
                                    + " object section of the model xml: "
                                    + model_xml_level3.tag
                                )
                                warnings.warn(msg)

                        self.template_specifications[model_xml_level2.attrib["id"]] = (
                            template_object
                        )
                    else:
                        msg = (
                            "Unknown entry while parsing the template section of the model xml: "
                ...
                for model_xml_level2 in model_xml_level1:
                    if model_xml_level2.tag.lower() == "kernel-width":
                        ...
                        model_xml_level2.tag.lower() == "number-of-interpolation-points"
                    ):
                        ...
                        model_xml_level2.tag.lower() == "metric-parameters-file"
                    ):  # for metric learning
                        self.metric_parameters_file = model_xml_level2.text
                    elif (
                        model_xml_level2.tag.lower() == "interpolation-points-file"
                    ):  # for metric learning
                        self.interpolation_points_file = model_xml_level2.text
                    elif (
                        model_xml_level2.tag.lower()
                        ...
                        msg = (
                            "Unknown entry while parsing the deformation-parameters section of the model xml: "
                            + model_xml_level2.tag
                        )
                        warnings.warn(msg)
            #
            ...
            #     self.use_exp_parallelization = self._on_off_to_bool(model_xml_level1.text)
                ...
                msg = (
                    "Unknown entry while parsing root of the model xml: "
                    + model_xml_level1.tag
                )
                warnings.warn(msg)
                        ...
                        if dataset_xml_level2.tag.lower() == "visit":
                            ...
                            for dataset_xml_level3 in dataset_xml_level2:
                                if dataset_xml_level3.tag.lower() == "filename":
                                    visit_filenames[
                                        dataset_xml_level3.attrib["object_id"]
                                    ] = os.path.normpath(
                                        os.path.join(
                                            data_set_xml_dirname,
                                            dataset_xml_level3.text,
                                        )
                                    )
    ...
    def _read_optimization_parameters_xml(self, optimization_parameters_xml_path):
        if (
            optimization_parameters_xml_path is not None
            and optimization_parameters_xml_path != "None"
        ):
            optimization_parameters_xml_level0 = et.parse(
                optimization_parameters_xml_path
            ).getroot()

            for (
                optimization_parameters_xml_level1
            ) in optimization_parameters_xml_level0:
                if (
                    optimization_parameters_xml_level1.tag.lower()
                    == "optimization-method-type"
                ):
                    self.optimization_method_type = (
                        optimization_parameters_xml_level1.text.lower()
                    )
                elif (
                    optimization_parameters_xml_level1.tag.lower()
                    ...
                    self.print_every_n_iters = int(
                        optimization_parameters_xml_level1.text
                    )
                elif (
                    optimization_parameters_xml_level1.tag.lower()
                    == "sample-every-n-mcmc-iters"
                ):
                    self.sample_every_n_mcmc_iters = int(
                        optimization_parameters_xml_level1.text
                    )
                elif (
                    optimization_parameters_xml_level1.tag.lower()
                    ...
                    self.use_sobolev_gradient = self._on_off_to_bool(
                        optimization_parameters_xml_level1.text
                    )
                elif (
                    optimization_parameters_xml_level1.tag.lower()
                    ...
                    self.sobolev_kernel_width_ratio = float(
                        optimization_parameters_xml_level1.text
                    )
                elif (
                    optimization_parameters_xml_level1.tag.lower()
                    ...
                    self.initial_step_size = float(
                        optimization_parameters_xml_level1.text
                    )
                elif (
                    optimization_parameters_xml_level1.tag.lower() == "freeze-template"
                ):
                    self.freeze_template = self._on_off_to_bool(
                        optimization_parameters_xml_level1.text
                    )
                elif (
                    optimization_parameters_xml_level1.tag.lower()
                    ...
                    self.freeze_control_points = self._on_off_to_bool(
                        optimization_parameters_xml_level1.text
                    )
                elif (
                    optimization_parameters_xml_level1.tag.lower()
                    == "freeze-principal-directions"
                ):
                    self.freeze_principal_directions = self._on_off_to_bool(
                        optimization_parameters_xml_level1.text
                    )
                elif optimization_parameters_xml_level1.tag.lower() == "gpu-mode":
                    self.gpu_mode = GpuMode[
                        optimization_parameters_xml_level1.text.upper()
                    ]
                    # if self.gpu_mode:
                    ...
                    self.max_line_search_iterations = int(
                        optimization_parameters_xml_level1.text
                    )
                elif optimization_parameters_xml_level1.tag.lower() == "state-file":
                    self.state_file = os.path.join(
                        os.path.dirname(optimization_parameters_xml_path),
                        optimization_parameters_xml_level1.text,
                    )
                elif (
                    optimization_parameters_xml_level1.tag.lower()
                    ...
                    self.use_rk2_for_shoot = self._on_off_to_bool(
                        optimization_parameters_xml_level1.text
                    )
                    ...
                    self.use_rk2_for_flow = self._on_off_to_bool(
                        optimization_parameters_xml_level1.text
                    )
                elif (
                    optimization_parameters_xml_level1.tag.lower()
                    == "momenta-proposal-std"
                ):
                    self.momenta_proposal_std = float(
                        optimization_parameters_xml_level1.text
                    )
                elif (
                    optimization_parameters_xml_level1.tag.lower()
                    == "onset-age-proposal-std"
                ):
                    self.onset_age_proposal_std = float(
                        optimization_parameters_xml_level1.text
                    )
                elif (
                    optimization_parameters_xml_level1.tag.lower()
                    == "acceleration-proposal-std"
                ):
                    self.acceleration_proposal_std = float(
                        optimization_parameters_xml_level1.text
                    )
                elif (
                    optimization_parameters_xml_level1.tag.lower()
                    == "sources-proposal-std"
                ):
                    self.sources_proposal_std = float(
                        optimization_parameters_xml_level1.text
                    )
                elif (
                    optimization_parameters_xml_level1.tag.lower()
                    ...
                    self.scale_initial_step_size = self._on_off_to_bool(
                        optimization_parameters_xml_level1.text
                    )
                elif (
                    optimization_parameters_xml_level1.tag.lower()
                    ...
                    self.initialization_heuristic = self._on_off_to_bool(
                        optimization_parameters_xml_level1.text
                    )
                elif optimization_parameters_xml_level1.tag.lower() == "freeze-v0":
                    self.freeze_v0 = self._on_off_to_bool(
                        optimization_parameters_xml_level1.text
                    )
                elif optimization_parameters_xml_level1.tag.lower() == "freeze-p0":
                    self.freeze_p0 = self._on_off_to_bool(
                        optimization_parameters_xml_level1.text
                    )
                elif (
                    optimization_parameters_xml_level1.tag.lower()
                    == "freeze-modulation-matrix"
                ):
                    self.freeze_modulation_matrix = self._on_off_to_bool(
                        optimization_parameters_xml_level1.text
                    )
                elif (
                    optimization_parameters_xml_level1.tag.lower()
                    == "freeze-reference-time"
                ):
                    self.freeze_reference_time = self._on_off_to_bool(
                        optimization_parameters_xml_level1.text
                    )
                elif (
                    optimization_parameters_xml_level1.tag.lower()
                    == "freeze-time-shift-variance"
                ):
                    self.freeze_time_shift_variance = self._on_off_to_bool(
                        optimization_parameters_xml_level1.text
                    )
                elif (
                    optimization_parameters_xml_level1.tag.lower()
                    == "freeze-acceleration-variance"
                ):
                    self.freeze_acceleration_variance = self._on_off_to_bool(
                        optimization_parameters_xml_level1.text
                    )
                elif (
                    optimization_parameters_xml_level1.tag.lower()
                    ...
                    optimization_parameters_xml_level1.tag.lower()
                    == "freeze-noise-variance"
                ):
                    self.freeze_noise_variance = self._on_off_to_bool(
                        optimization_parameters_xml_level1.text
                    )
                elif (
                    optimization_parameters_xml_level1.tag.lower()
                    == "freeze-translation-vectors"
                ):
                    self.freeze_translation_vectors = self._on_off_to_bool(
                        optimization_parameters_xml_level1.text
                    )
                elif (
                    optimization_parameters_xml_level1.tag.lower()
                    == "freeze-rotation-angles"
                ):
                    self.freeze_rotation_angles = self._on_off_to_bool(
                        optimization_parameters_xml_level1.text
                    )
                elif (
                    optimization_parameters_xml_level1.tag.lower()
                    == "freeze-scaling-ratios"
                ):
                    self.freeze_scaling_ratios = self._on_off_to_bool(
                        optimization_parameters_xml_level1.text
                    )
                elif (
                    optimization_parameters_xml_level1.tag.lower()
                    ...
                    self.gradient_based_estimator = (
                        optimization_parameters_xml_level1.text
                    )

                else:
                    msg = (
                        "Unknown entry while parsing the optimization_parameters xml: "
