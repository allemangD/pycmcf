...
class DeformableObjectReader:
        ...
        if object_type.lower() in [
            ...
            if object_filename.find(".png") > 0:
                ...
                if len(img_data.shape) > 2:
                    warnings.warn(
                        "Multi-channel images are not managed (yet). Defaulting to the first channel."
                    )
                    dimension = 2
                    img_data = img_data[:, :, 0]
                ...
                object_filename.find(".nii") > 0 or object_filename.find(".nii.gz") > 0
            ):
                ...
                assert len(img_data.shape) == 3, (
                    "Multi-channel images not available (yet!)."
                )
    ...
    def read_file(filename, dimension=None, extract_connectivity=False):
        ...
        assert dimension in [2, 3], (
            "The ambient-space dimension should be either 2 ou 3."
        )
        ...
        if not extract_connectivity:
            ...
            lines = nps.vtk_to_numpy(poly_data.GetLines().GetData()).reshape((-1, 3))[
                :, 1:
            ]
            polygons = nps.vtk_to_numpy(poly_data.GetPolys().GetData()).reshape(
                (-1, 4)
            )[:, 1:]
