...
def compute_parallel_transport(
    ...
    **kwargs,
    ...
    control_points = utilities.move_data(
        control_points, dtype=tensor_scalar_type, device=device
    )
    ...
    if need_to_project_initial_momenta:
        ...
        """
        velocity = utilities.move_data(
            velocity, dtype=tensor_scalar_type, device="cpu"
        )  # TODO: could this be done on gpu ?
        kernel_matrix = utilities.move_data(
            kernel_matrix, dtype=tensor_scalar_type, device="cpu"
        )  # TODO: could this be done on gpu ?
        ...
        projected_momenta = (
            torch.cholesky_solve(velocity, cholesky_kernel_matrix, upper=True)
            .squeeze()
            .contiguous()
        )
    ...
    """
    device, _ = utilities.get_best_device(gpu_mode)
    projected_momenta = utilities.move_data(
        projected_momenta, dtype=tensor_scalar_type, device=device
    )
    ...
    template_points = {
        key: utilities.move_data(value, dtype=tensor_scalar_type, device=device)
    ...
    assert math.fabs(tmin) != float("inf"), (
        "Please specify a minimum time for the geodesic trajectory"
    )
    assert math.fabs(tmax) != float("inf"), (
        "Please specify a maximum time for the geodesic trajectory"
    )
    ...
    for i, (time, cp, mom, transported_mom) in enumerate(
        ...
        template.write(
            output_dir,
            names,
            {key: value.detach().cpu().numpy() for key, value in parallel_data.items()},
        )
