...
def estimate_geodesic_regression_for_subject(
    ...
    deformetrica,
    xml_parameters,
    regressions_output_path,
    global_full_dataset_filenames,
    global_full_visit_ages,
    global_full_subject_ids,
):
    logger.info("")
    logger.info(
        "[ geodesic regression for subject " + global_full_subject_ids[i] + " ]"
    )
    logger.info("")

    # Create folder.
    subject_regression_output_path = os.path.join(
        regressions_output_path,
        "GeodesicRegression__subject_" + global_full_subject_ids[i],
    )
    if os.path.isdir(subject_regression_output_path):
        shutil.rmtree(subject_regression_output_path)
...
def shoot(
    ...
    exponential = Exponential(
        ...
        initial_control_points=control_points_torch,
        initial_momenta=momenta_torch,
    )
    exponential.shoot()
    return exponential.control_points_t[
        -1
    ].detach().cpu().numpy(), exponential.momenta_t[-1].detach().cpu().numpy()


def reproject_momenta(
    ...
    target_momenta_torch = torch.cholesky_solve(
        kernel.convolve(
            target_control_points_torch,
            source_control_points_torch,
            source_momenta_torch,
        ),
        torch.cholesky(
            kernel.get_kernel_matrix(target_control_points_torch), upper=True
        ),
        upper=True,
    )
...
def parallel_transport(
    ...
    exponential = Exponential(
        ...
        initial_control_points=source_control_points_torch,
        initial_momenta=driving_momenta_torch,
    )
    exponential.shoot()
    ...
    return (
        transported_control_points_torch.detach().cpu().numpy(),
        transported_momenta_torch.detach().cpu().numpy(),
    )


def initialize_longitudinal_atlas(
    model_xml_path,
    dataset_xml_path,
    ...
    _template_specifications, model_options, _estimator_options = (
        global_deformetrica.further_initialization(
            "LongitudinalAtlas",
            xml_parameters.template_specifications,
            get_model_options(xml_parameters),
    ...
    global_total_number_of_observations = sum(
        [len(elt) for elt in global_full_visit_ages]
    )

    global_initial_control_points_are_given = (
        xml_parameters.initial_control_points is not None
    )
    global_initial_momenta_are_given = xml_parameters.initial_momenta is not None
    global_initial_modulation_matrix_is_given = (
        xml_parameters.initial_modulation_matrix is not None
    )
    global_initial_t0_is_given = xml_parameters.t0 is not None

    global_t0 = xml_parameters.t0
    if not global_initial_t0_is_given:
        global_t0 = sum([sum(elt) for elt in global_full_visit_ages]) / float(
            global_total_number_of_observations
        )

    global_tmin = sum([elt[0] for elt in global_full_visit_ages]) / float(
        global_number_of_subjects
    )
    ...
    if not global_overwrite and os.path.isdir(atlas_output_path):
        global_initial_control_points = read_2D_array(
            os.path.join(
                global_path_to_data, "ForInitialization__ControlPoints__FromAtlas.txt"
            )
        )
        global_atlas_momenta = read_3D_array(
            os.path.join(
                atlas_output_path,
        ...
        xml_parameters.dataset_filenames = [
            [elt[0]] for elt in xml_parameters.dataset_filenames
        ]
        ...
        if atlas_type == "Bayesian":
            model, global_atlas_momenta = estimate_bayesian_atlas(
                global_deformetrica, xml_parameters
            )
            global_objects_noise_std = [
                math.sqrt(elt) for elt in model.get_noise_variance()
            ]

        elif atlas_type == "Deterministic":
            model = estimate_deterministic_atlas(global_deformetrica, xml_parameters)
            global_objects_noise_std = [
                math.sqrt(elt) for elt in model.objects_noise_variance
            ]
        ...
        (
            ...
            original_objects_noise_variance,
        ...
        for k, (
            ...
            original_object_noise_variance,
        ) in enumerate(
            zip(
                global_objects_name,
                global_objects_name_extension,
            ...
            global_initial_objects_template_type.append(
                global_initial_template.object_list[k].type.lower()
            )
            ...
            estimated_template_path = os.path.join(
                atlas_output_path,
                atlas_type
                + "Atlas__EstimatedParameters__Template_"
                + object_name
                + object_name_extension,
            )
            global_initial_objects_template_path.append(
                os.path.join(
                    global_path_to_data,
                    "ForInitialization__Template_"
                    + object_name
                    + "__FromAtlas"
                    + object_name_extension,
                )
            ...
            if global_initial_objects_template_type[k] == "PolyLine".lower():
                cmd = (
                    "sed -i -- s/POLYGONS/LINES/g "
                    + global_initial_objects_template_path[k]
                )
        ...
        if global_initial_control_points_are_given:
            ...
            global_initial_control_points = read_2D_array(
                global_initial_control_points_path
            )
        else:
            ...
            shutil.copyfile(
                estimated_control_points_path, global_initial_control_points_path
            )
    ...
    regressions_output_path = os.path.join(
        preprocessings_folder, "2_individual_geodesic_regressions"
    )
    if not global_overwrite and os.path.isdir(regressions_output_path):
        global_initial_momenta = read_3D_array(
            os.path.join(
                global_path_to_data, "ForInitialization__Momenta__FromRegressions.txt"
            )
        )
    ...
    # Check if an initial (longitudinal) momenta is available.
        ...
        # Warning.
        if (
            not global_initial_control_points_are_given
            and global_initial_momenta_are_given
        ):
            msg = (
                "Initial momenta are given but not the corresponding initial control points. "
        ...
        if os.path.isdir(regressions_output_path):
            shutil.rmtree(regressions_output_path)
        ...
        for i in range(global_number_of_subjects):
            ...
            for k, (object_id, object_specs) in enumerate(
                xml_parameters.template_specifications.items()
            ):
            ...
            registration_control_points, registration_momenta = shoot(
                ...
                global_atlas_momenta[i],
                global_deformation_kernel_width,
                global_deformation_kernel_type,
                global_deformation_kernel_device,
                number_of_time_points=global_number_of_time_points,
                dense_mode=global_dense_mode,
            ...
            # Regression.
            regression_control_points, regression_momenta = (
                estimate_geodesic_regression_for_subject(
                    i,
                    global_deformetrica,
                    xml_parameters,
                    regressions_output_path,
            ...
            np.savetxt(
                path_to_subject_transported_regression_momenta,
                transported_regression_momenta,
            )
    ...
    kernel = kernel_factory.factory(
        "torch", kernel_width=xml_parameters.deformation_kernel_width
    )

    global_initial_control_points_torch = torch.from_numpy(
        global_initial_control_points
    ).type(global_tensor_scalar_type)

    global_initial_momenta_torch = torch.from_numpy(global_initial_momenta).type(
        global_tensor_scalar_type
    )
    global_initial_momenta_norm_squared = (
        torch.dot(
            global_initial_momenta_torch.view(-1),
            kernel.convolve(
                global_initial_control_points_torch,
                global_initial_control_points_torch,
                global_initial_momenta_torch,
            ).view(-1),
        )
        .detach()
        .cpu()
    ...
    for i in range(global_number_of_subjects):
        ...
        subject_regression_momenta = read_3D_array(
            path_to_subject_transported_regression_momenta
        )
        subject_regression_momenta_torch = torch.from_numpy(
            subject_regression_momenta
        ).type(global_tensor_scalar_type)

        subject_regression_momenta_scalar_product_with_population_momenta = (
            torch.dot(
                global_initial_momenta_torch.view(-1),
                kernel.convolve(
                    global_initial_control_points_torch,
                    global_initial_control_points_torch,
                    subject_regression_momenta_torch,
                ).view(-1),
            )
            .detach()
            .cpu()
    ...
    def get_acceleration_std_from_accelerations(
        accelerations,
        ...
        std_old, std_new = (
            math.sqrt(ss / float(number_of_subjects)),
            math.sqrt(ss / float(number_of_subjects)),
        )
        for iteration in range(max_number_of_iterations):
            ...
            std_new = 1.0 / math.sqrt(
                number_of_subjects * (1 - (phi / std_old) / (1 - Phi)) / ss
            )
            ...
            if iteration == max_number_of_iterations:
                msg = (
                    "When initializing the acceleration std parameter from the empirical std, the fixed-point "
    ...
    heuristic_initial_acceleration_std = get_acceleration_std_from_accelerations(
        heuristic_initial_accelerations
    )

    # Rescaling the initial momenta according to the mean of the acceleration factors.
    expected_mean_acceleration = float(
        truncnorm.stats(
            -1.0 / heuristic_initial_acceleration_std,
            float("inf"),
            loc=1.0,
            scale=heuristic_initial_acceleration_std,
            moments="m",
        )
    )
    ...
    # Initial momenta.
    global_initial_momenta_path = os.path.join(
        global_path_to_data, "ForInitialization__Momenta__RescaledWithHeuristics.txt"
    )
    np.savetxt(global_initial_momenta_path, global_initial_momenta)
    ...
    if (
        not global_initial_control_points_are_given
        and not global_initial_momenta_are_given
    ):
        logger.info("")
        ...
        # Shoot --------------------------------------------------------------------------------------------------------
        # Create folder.
        shooting_output_path = os.path.join(
            preprocessings_folder, "3_shooting_from_baseline_to_average"
        )
        if os.path.isdir(shooting_output_path):
            shutil.rmtree(shooting_output_path)
        ...
        geodesic.set_template_points_t0(
            {
                key: Variable(
                    torch.from_numpy(value).type(global_tensor_scalar_type),
                    requires_grad=False,
                )
        ...
        if global_dense_mode:
            geodesic.set_control_points_t0(
                geodesic.get_template_points_t0()["landmark_points"]
            )
        else:
            geodesic.set_control_points_t0(
                Variable(
                    torch.from_numpy(global_initial_control_points).type(
                        global_tensor_scalar_type
                    )
                )
            )
        geodesic.set_momenta_t0(
            Variable(
                torch.from_numpy(global_initial_momenta).type(
                    global_tensor_scalar_type
                ),
                requires_grad=False,
            )
        ...
        geodesic.write(
            ...
            global_deformetrica.output_dir,
            write_adjoint_parameters=True,
        )

        # Export results -----------------------------------------------------------------------------------------------
        number_of_timepoints = (
            geodesic.forward_exponential.number_of_time_points
            + geodesic.backward_exponential.number_of_time_points
            - 2
        )
        ...
        for k, (object_name, object_name_extension) in enumerate(
            ...
            global_initial_objects_template_path[k] = os.path.join(
                ...
                "ForInitialization__Template_"
                + object_name
                + "__FromAtlasAndShooting"
                + object_name_extension,
            )
            shutil.copyfile(
                shooted_template_path, global_initial_objects_template_path[k]
            )
        ...
        global_initial_momenta_path = os.path.join(
            global_path_to_data,
            "ForInitialization__Momenta__FromRegressionsAndShooting.txt",
        )
    ...
    if not (
        global_initial_control_points_are_given
        and global_initial_modulation_matrix_is_given
    ):
        logger.info("")
        ...
        if (
            ...
            msg = (
                "Initial modulation matrix is given but not the corresponding initial control points. "
        ...
        xml_parameters._read_model_xml(model_xml_path)
        xml_parameters._read_dataset_xml(dataset_xml_path)
        xml_parameters._read_optimization_parameters_xml(
            optimization_parameters_xml_path
        )
        ...
        K = np.zeros(
            (number_of_control_points * dimension, number_of_control_points * dimension)
        )
        for i in range(number_of_control_points):
            for j in range(number_of_control_points):
                ...
                kernel_distance = math.exp(
                    -np.sum((cp_j - cp_i) ** 2)
                    / (xml_parameters.deformation_kernel_width**2)
                )
        ...
        for i in range(momenta.shape[0]):
            w.append(
                momenta[i].ravel()
                - np.dot(momenta[i].ravel(), Km) / mKm * global_initial_momenta.ravel()
            )
        w = np.array(w)
        ...
        if xml_parameters.number_of_sources is not None:
            ...
            number_of_sources = read_2D_array(
        ...
        for i in range(global_number_of_subjects):
            residuals.append(
                w[i]
                - np.dot(global_initial_modulation_matrix, global_initial_sources[i])
            )
        mean_relative_residual = np.mean(np.absolute(np.array(residuals))) / np.mean(
            np.absolute(w)
        )
        logger.info(
            ">> Mean relative residual: %.3f %%." % (100 * mean_relative_residual)
        )

        # Save.
        global_initial_modulation_matrix_path = os.path.join(
            global_path_to_data, "ForInitialization__ModulationMatrix__FromICA.txt"
        )
        np.savetxt(
            global_initial_modulation_matrix_path, global_initial_modulation_matrix
        )

        global_initial_sources_path = os.path.join(
            global_path_to_data, "ForInitialization__Sources__FromICA.txt"
        )
    ...
    # Clean folder.
    registration_output_path = os.path.join(
        preprocessings_folder, "4_longitudinal_registration"
    )
            ...
            if len(registrations) > 0:
                shutil.rmtree(
                    os.path.join(
                        ...
                        os.listdir(registration_output_path)[-1],
                    )
                )
    ...
    # Launch.
    estimate_longitudinal_registration(
        global_deformetrica, xml_parameters, overwrite=global_overwrite
    )
    ...
    estimated_accelerations_path = os.path.join(
        registration_output_path,
        "LongitudinalRegistration__EstimatedParameters__Accelerations.txt",
    )
    ...
    global_acceleration_std = get_acceleration_std_from_accelerations(
    ...
    expected_mean_acceleration = float(
        truncnorm.stats(
            -1.0 / global_acceleration_std,
            float("inf"),
            loc=1.0,
            scale=global_acceleration_std,
            moments="m",
        )
    )
    ...
    global_initial_momenta_path = os.path.join(
        global_path_to_data,
        "ForInitialization__Momenta__RescaledWithLongitudinalRegistration.txt",
    )
    np.savetxt(global_initial_momenta_path, global_initial_momenta)
    ...
    global_initial_accelerations_path = os.path.join(
        global_path_to_data,
        "ForInitialization__Accelerations__FromLongitudinalRegistration.txt",
    )
    ...
    if os.path.isdir(longitudinal_atlas_output_path):
        shutil.rmtree(longitudinal_atlas_output_path)
    ...
    for k, (object_name, object_name_extension) in enumerate(
        ...
        estimated_template_path = os.path.join(
            ...
            % (
                object_name,
                model.spatiotemporal_reference_frame.geodesic.backward_exponential.number_of_time_points
                - 1,
    ...
    shutil.copyfile(
        estimated_modulation_matrix_path, global_initial_modulation_matrix_path
    )
    model_xml_level0 = insert_model_xml_level1_entry(
        model_xml_level0,
        "initial-modulation-matrix",
    ...
    global_initial_onset_ages_path = os.path.join(
        global_path_to_data, "ForInitialization__OnsetAges__FromLongitudinalAtlas.txt"
    )
    shutil.copyfile(estimated_onset_ages_path, global_initial_onset_ages_path)
    model_xml_level0 = insert_model_xml_level1_entry(
        model_xml_level0, "initial-onset-ages", global_initial_onset_ages_path
    ...
    estimated_accelerations_path = os.path.join(
        longitudinal_atlas_output_path,
        "LongitudinalAtlas__EstimatedParameters__Accelerations.txt",
    )
    global_initial_accelerations_path = os.path.join(
        global_path_to_data,
        "ForInitialization__Accelerations__FromLongitudinalAtlas.txt",
    )
    ...
    global_initial_sources_path = os.path.join(
        global_path_to_data, "ForInitialization__Sources__FromLongitudinalAtlas.txt"
    )
    shutil.copyfile(estimated_sources_path, global_initial_sources_path)
    model_xml_level0 = insert_model_xml_level1_entry(
        model_xml_level0, "initial-sources", global_initial_sources_path
