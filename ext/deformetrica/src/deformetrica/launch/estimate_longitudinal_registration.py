...
def estimate_longitudinal_registration_for_subject(
    ...
    full_subject_ids,
    full_dataset_filenames,
    full_visit_ages,
    global_dimension,
    global_tensor_scalar_type,
    global_tensor_integer_type,
    overwrite=True,
):
    """
    ...
    dataset = create_dataset(
        template_specifications, dimension=global_dimension, **dataset_specifications
    )
    ...
    subject_registration_output_path = os.path.join(
        registration_output_path,
        "LongitudinalRegistration__subject_" + full_subject_ids[i],
    )
    ...
    estimator_options["state_file"] = os.path.join(
        subject_registration_output_path, "deformetrica-state.p"
    )
    ...
    individual_RER = model.initialize_random_effects_realization(
    ...
    if (
        ...
        estimator = GradientAscent(
            model,
            dataset,
            output_dir=subject_registration_output_path,
            **estimator_options,
        )
    elif estimator_options["optimization_method_type"].lower() in [
        "ScipyLBFGS".lower(),
        "ScipyPowell".lower(),
    ]:
    ...
    try:
        ...
        model._write_model_parameters(
            estimator.individual_RER, subject_registration_output_path
        )
        end_time = time.time()
        ...
        if not (
            estimator.name.lower() == "scipyoptimize"
            and estimator.method.lower() == "powell"
        ):
            ...
            estimator = ScipyOptimize(
                ...
                optimization_method_type="ScipyPowell",
                max_iterations=estimator_options["max_iterations"],
...
def estimate_longitudinal_registration(
    ...
    estimator_options["save_every_n_iters"] = (
        100000  # Don't waste time saving intermediate results.
    )

    # Global parameter.
    initial_modulation_matrix_shape = read_2D_array(
