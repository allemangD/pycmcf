...
from ..core.model_tools.manifolds.generic_spatiotemporal_reference_frame import (
    GenericSpatiotemporalReferenceFrame,
)
...
def _initialize_parametric_exponential(
    model, xml_parameters, dataset, exponential_factory, metric_parameters
):
    """
    ...
    """
    metric_parameters = np.reshape(
        metric_parameters,
        (
            len(metric_parameters),
            int(Settings().dimension * (Settings().dimension + 1) / 2),
        ),
    )
    ...
    if xml_parameters.interpolation_points_file is None:
        ...
        numpy_observations = np.concatenate(
            [elt.cpu().data.numpy() for elt in dataset.deformable_objects]
        )
        numpy_observations = numpy_observations.reshape(
            len(numpy_observations), dimension
        )
        ...
        logger.info(
            "Loading the interpolation points from file",
            xml_parameters.interpolation_points_file,
        )
        ...
        interpolation_points = interpolation_points.reshape(
            len(interpolation_points), Settings().dimension
        )
    ...
    if metric_parameters is None:
        ...
        metric_parameters = np.zeros(
            (model.number_of_interpolation_points, int(dim * (dim + 1) / 2))
        )
        val = np.sqrt(1.0 / model.number_of_interpolation_points)
        ...
        assert len(metric_parameters) == len(interpolation_points), (
            "Bad input format for the metric parameters"
        )
        assert (
            len(metric_parameters[0])
            == Settings().dimension * (Settings().dimension + 1) / 2
        ), "Bad input format for the metric parameters"

    # Parameters of the parametric manifold:
    manifold_parameters = {}
    logger.info("The width for the metric interpolation is set to", width)
    manifold_parameters["number_of_interpolation_points"] = (
        model.number_of_interpolation_points
    )
    manifold_parameters["width"] = width

    manifold_parameters["interpolation_points_torch"] = Variable(
        torch.from_numpy(interpolation_points).type(Settings().tensor_scalar_type),
        requires_grad=False,
    )
    manifold_parameters["interpolation_values_torch"] = Variable(
        torch.from_numpy(metric_parameters).type(Settings().tensor_scalar_type)
    )
...
def initialize_spatiotemporal_reference_frame(
    model, xml_parameters, dataset, observation_type="image"
):
    """
    Initialize everything which is relative to the geodesic its parameters.
    """
    assert xml_parameters.dimension is not None, (
        "Provide a dimension for the longitudinal metric learning atlas."
    )
    ...
    if xml_parameters.metric_parameters_file is not None:
        logger.info(
            "Loading metric parameters from file", xml_parameters.metric_parameters_file
        )
    ...
    if Settings().dimension == 1:
        ...
        xml_parameters.number_of_sources == 0
        or xml_parameters.number_of_sources is None
    ):
        logger.info(
...
def instantiate_longitudinal_metric_model(
    xml_parameters, dataset=None, number_of_subjects=None, observation_type="scalar"
):
    model = LongitudinalMetricLearning()

    model.observation_type = (
        observation_type  # TODO : replace this with use of the template object.
    )

    template = dataset.deformable_objects[0][
        0
    ]  # because we only care about its 'metadata'
    ...
    model.is_frozen["log_acceleration_variance"] = (
        xml_parameters.freeze_log_acceleration_variance
    )
    ...
    assert not (dataset is None and number_of_subjects is None), (
        "Provide at least one info"
    )
    ...
    if xml_parameters.initial_onset_ages is not None:
        logger.info(
            "Setting initial onset ages from", xml_parameters.initial_onset_ages, "file"
        )
        onset_ages = read_2D_array(xml_parameters.initial_onset_ages).reshape(
            (len(dataset.times),)
        )
    ...
    initialize_spatiotemporal_reference_frame(
        model, xml_parameters, dataset, observation_type=observation_type
    )
    ...
    if xml_parameters.initial_modulation_matrix is not None:
        ...
        if len(modulation_matrix.shape) == 1:
            ...
            modulation_matrix = modulation_matrix.reshape(
                xml_parameters.latent_space_dimension,
                xml_parameters.latent_space_dimension - 1,
            )
        logger.info(
            ">> Reading "
            + str(modulation_matrix.shape[1])
            + "-source initial modulation matrix from file: "
            + xml_parameters.initial_modulation_matrix
        )
        assert xml_parameters.number_of_sources == modulation_matrix.shape[1], (
            "Please set correctly the number of sources"
        )
    ...
    if xml_parameters.initial_sources is not None:
        logger.info(
            "Setting initial sources from", xml_parameters.initial_sources, "file"
        )
        individual_RER["sources"] = read_2D_array(
            xml_parameters.initial_sources
        ).reshape(len(dataset.times), model.number_of_sources)

    elif model.number_of_sources > 0:
        logger.info("Initializing all sources to zero")
        individual_RER["sources"] = np.zeros(
            (number_of_subjects, model.number_of_sources)
        )
    ...
    if dataset is not None:
        ...
        if model.get_noise_variance() is None:
            ...
            onset_ages, log_accelerations, sources = (
                model._individual_RER_to_torch_tensors(individual_RER, False)
            )
        ...
        if model.get_noise_variance() is None:
            raise RuntimeError(
                "I can't initialize the initial noise variance: no dataset and no initialization given."
            )
...
def estimate_longitudinal_metric_model(xml_parameters):
    ...
    if xml_parameters.optimization_method_type == "GradientAscent".lower():
        ...
        sampler.individual_proposal_distributions["onset_age"] = (
            onset_age_proposal_distribution
        )
        ...
        sampler.individual_proposal_distributions["log_acceleration"] = (
            log_acceleration_proposal_distribution
        )
        ...
        if model.number_of_sources > 0:
            ...
            sampler.individual_proposal_distributions["sources"] = (
                sources_proposal_distribution
            )
        ...
        estimator.gradient_based_estimator.initial_step_size = (
        ...
        msg = (
            'Unknown optimization-method-type: "'
