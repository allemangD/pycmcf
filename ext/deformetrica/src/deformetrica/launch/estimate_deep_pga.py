...
def run_model_on_MNIST(latent_space_dimension):
    ...
    np.savetxt(
        os.path.join(Settings().output_dir, "labels_train.txt"), np.array(train_labels)
    )
    np.savetxt(
        os.path.join(Settings().output_dir, "labels_test.txt"), np.array(test_labels)
    )

    train_dataset.update()
    test_dataset.update()

    train_images_data = np.array(
        [
            elt[0].object_list[0].get_intensities()
            for elt in train_dataset.deformable_objects
        ]
    )
    test_images_data = np.array(
        [
            elt[0].object_list[0].get_intensities()
            for elt in test_dataset.deformable_objects
        ]
    )
    ...
    latent_space_positions = pca.fit_transform(
        [elt.flatten() for elt in train_images_data]
    )
    ...
    reconstruction_error_train = mean_squared_error(
        reconstructed, [elt.flatten() for elt in train_images_data]
    )
    reconstruction_error_test = mean_squared_error(
        reconstructed_test, [elt.flatten() for elt in test_images_data]
    )
    ...
    np.savetxt(
        os.path.join(Settings().output_dir, "pca_reconstruction_error.txt"), to_write
    )

    # We now normalize every latent_space_positions
    for i in range(latent_space_dimension):
        latent_space_positions[:, i] = (
            latent_space_positions[:, i] - np.mean(latent_space_positions[:, i])
        ) / np.std(latent_space_positions[:, i])

    latent_space_positions_torch = torch.from_numpy(latent_space_positions).type(
        Settings().tensor_scalar_type
    )
    ...
    train_dataset_fit = TensorDataset(
        latent_space_positions_torch, train_images_data_torch
    )
    ...
    register_test_to_model(
        model,
        test_dataset,
        "test_output_" + str(latent_space_dimension),
        latent_space_dimension,
    )
...
def run_on_cylinder():
    ...
    train_set = np.array(
        [[np.cos(elt[0]), np.sin(elt[0]), elt[1]] for elt in train_coords]
    )
    test_set = np.array(
        [[np.cos(elt[0]), np.sin(elt[0]), elt[1]] for elt in test_coords]
    )

    np.save(
        os.path.join(Settings().output_dir, "train_latent_coordinates.npy"),
        train_coords,
    )
    np.save(
        os.path.join(Settings().output_dir, "test_latent_coordinates.npy"), test_coords
    )
...
def instantiate_and_estimate_model(
    dataset,
    latent_space_dimension,
    initial_lds_positions,
    initial_net,
    ...
    model.individual_random_effects["latent_position"].mean = np.zeros(
        (latent_space_dimension,)
    )
    ...
    sampler.individual_proposal_distributions["latent_position"] = (
        latent_position_proposal_distribution
    )
