...
from ..launch.estimate_longitudinal_metric_model import (
    instantiate_longitudinal_metric_model,
)


def estimate_longitudinal_registration_for_subject(args):
    i, general_settings, xml_parameters, registration_output_path, full_dataset = args

    Settings().initialize(general_settings)

    logger.info("")
    logger.info(
        "[ longitudinal registration of subject " + full_dataset.subject_ids[i] + " ]"
    )
    logger.info("")
    ...
    subject_registration_output_path = os.path.join(
        ...
        "LongitudinalMetricRegistration__subject_" + full_dataset.subject_ids[i],
    )
    ...
    Settings().state_file = os.path.join(
        subject_registration_output_path, "pydef_state.p"
    )
    ...
    for xml_parameter, random_effect_name in zip(
        [xml_parameters.initial_onset_ages, xml_parameters.initial_log_accelerations],
        ["onset_age", "log_acceleration"],
    ):
        if (
            xml_parameter is not None
            and len(individual_RER[random_effect_name].shape) > 1
        ):
...
def estimate_longitudinal_metric_registration(xml_parameters):
    ...
    if full_dataset is None:
        full_dataset = read_and_create_image_dataset(
    ...
    xml_parameters.save_every_n_iters = (
        100000  # Don't waste time saving intermediate results.
    )

    assert not xml_parameters.initialization_heuristic, (
        "Should not be used for registrations."
    )
    ...
    for i in range(number_of_subjects):
        ...
        onset_ages[i] = np.loadtxt(
            os.path.join(
                subject_registration_output_path,
                "LongitudinalMetricRegistration_onset_ages.txt",
            )
        )
        log_accelerations[i] = np.loadtxt(
            os.path.join(
                subject_registration_output_path,
                "LongitudinalMetricRegistration_log_accelerations.txt",
            )
        )

        sources[i, :] = np.loadtxt(
            os.path.join(
                subject_registration_output_path,
                "LongitudinalMetricRegistration_sources.txt",
            )
        )
    ...
    path_to_log_accelerations = os.path.join(
        temporary_output_path, "log_acceleration.txt"
    )
    ...
    model, _ = instantiate_longitudinal_metric_model(
        xml_parameters, full_dataset, observation_type="image"
    )
    model.name = "LongitudinalRegistration"
    model.write(
        full_dataset, None, individual_RER, sample=False, update_fixed_effects=False
    )
