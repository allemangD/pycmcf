...
class ProfileAttachments:
    def __init__(
        self, kernel_type, kernel_device="CPU", use_cuda=False, data_size="small"
    ):
        ...
        self.multi_object_attachment = MultiObjectAttachment(
            ["varifold"],
            [kernel_factory.factory(kernel_type, kernel_width, device=kernel_device)],
        )

        self.kernel = kernel_factory.factory(
        ...
        if data_size == "small":
            self.surface_mesh_1 = reader.create_object(
                path_to_small_surface_mesh_1, "SurfaceMesh", tensor_scalar_type
            )
            self.surface_mesh_2 = reader.create_object(
                path_to_small_surface_mesh_2, "SurfaceMesh", tensor_scalar_type
            )
            ...
            self.surface_mesh_1 = reader.create_object(
                path_to_large_surface_mesh_1, "SurfaceMesh", tensor_scalar_type
            )
            self.surface_mesh_2 = reader.create_object(
                path_to_large_surface_mesh_2, "SurfaceMesh", tensor_scalar_type
            )
            self.surface_mesh_1_points = tensor_scalar_type(
                self.surface_mesh_1.get_points()
            )
            ...
            connectivity = np.array(
                list(itertools.combinations(range(100), 3))[:data_size]
            )  # up to ~16k.
    ...
    for data_size in ["100", "200", "400", "800", "1600", "3200", "6400"]:
        for attachment_type in ["varifold", "current"]:
            for kernel_type in [
                ...
                method_to_run.append(
                    (data_size, attachment_type + "_attachment_with_backward")
                )
    ...
    for setup in build_setup:
        ...
        np.save(
            os.path.join(
                ...
                "results_profile_attachments.npy",
            ),
            np.array(results),
        )
