...
def _initialize_modulation_matrix_and_sources(dataset, p0, v0, number_of_sources):
    ...
    logger.info(
        "Performing principal component analysis on the orthogonal variations, for initialization of A and s_i."
    )
    ...
    sources = []
    for elt in dataset.deformable_objects:
        obs_for_subject = np.array(
            [(im.cpu().data.numpy() - p0).flatten() for im in elt]
        )
...
def _smart_initialization_individual_effects(dataset):
    ...
    """
    logger.info(
        "Performing initial least square regressions on the subjects, for initialization purposes."
    )
    ...
    for i in range(number_of_subjects):
        ...
        if len(dataset.times[i]) <= 1:
            ...
            data_for_subject = np.array(
                [
                    elt.cpu().data.numpy().flatten()
                    for elt in dataset.deformable_objects[i]
                ]
            )
...
def _smart_initialization(dataset, number_of_sources, observation_type):
    ...
    for i in range(dataset.number_of_subjects):
        aux = np.mean(
            np.array([elt.cpu().data.numpy() for elt in dataset.deformable_objects[i]]),
            0,
        )
        p0 += aux
    ...
    for i in range(len(ais)):
        ...
        alpha = max(0.003, min(10.0, alpha_proposal))
        alphas.append(alpha)

        onset_age_proposal = (
            1.0
            / alpha
            * np.dot(p0.flatten() - bis[i].flatten(), v0.flatten())
            / np.sum(v0**2)
        )
        # onset_age_proposal = np.linalg.norm(p0-bis[i])/np.linalg.norm(ais[i])
        onset_age = max(
            reference_time - 2 * std_obs,
            min(reference_time + 2 * std_obs, onset_age_proposal),
        )
    ...
    if True:
        ...
        log_accelerations = (
            0.5
            * (log_accelerations - np.mean(log_accelerations, 0))
            / np.std(log_accelerations, 0)
        )
        alphas = np.exp(log_accelerations)
        # We want the onset ages to have an std equal to the std of the obser times

        onset_ages = (onset_ages - np.mean(onset_ages, 0)) / np.std(
            onset_ages, 0
        ) * std_obs + np.mean(onset_ages)
        logger.info("std onset_ages vs obs times", np.std(onset_ages), std_obs)

    reference_time = np.mean(onset_ages, 0)

    if number_of_sources > 0:
        modulation_matrix, sources = _initialize_modulation_matrix_and_sources(
            dataset, p0, v0, number_of_sources
        )
...
if __name__ == "__main__":
    ...
    smart_initialization_output_path = os.path.join(
        preprocessings_folder, "1_smart_initialization"
    )
    ...
    if (
        xml_parameters.number_of_sources is None
        or xml_parameters.number_of_sources == 0
    ):
        (
            reference_time,
            average_a,
            ...
            modulation_matrix,
            sources,
        ) = _smart_initialization(dataset, 0, observation_type)
    else:
        (
            reference_time,
            average_a,
            ...
            modulation_matrix,
            sources,
        ) = _smart_initialization(
            dataset, xml_parameters.number_of_sources, observation_type
        )
    ...
    xml_parameters.initial_log_accelerations = os.path.join(
        smart_initialization_output_path, "SmartInitialization_log_accelerations.txt"
    )

    write_2D_array(onset_ages, "SmartInitialization_onset_ages.txt")
    xml_parameters.initial_onset_ages = os.path.join(
        smart_initialization_output_path, "SmartInitialization_onset_ages.txt"
    )

    if xml_parameters.exponential_type != "deep":
        write_2D_array(np.array([p0]), "SmartInitialization_p0.txt")
        xml_parameters.p0 = os.path.join(
            smart_initialization_output_path, "SmartInitialization_p0.txt"
        )

        write_2D_array(np.array([average_a]), "SmartInitialization_v0.txt")
        xml_parameters.v0 = os.path.join(
            smart_initialization_output_path, "SmartInitialization_v0.txt"
        )
    ...
    if modulation_matrix is not None:
        ...
        if xml_parameters.exponential_type != "deep":
            write_2D_array(
                modulation_matrix, "SmartInitialization_modulation_matrix.txt"
            )
            xml_parameters.initial_modulation_matrix = os.path.join(
                smart_initialization_output_path,
                "SmartInitialization_modulation_matrix.txt",
            )
        write_2D_array(sources, "SmartInitialization_sources.txt")
        xml_parameters.initial_sources = os.path.join(
            smart_initialization_output_path, "SmartInitialization_sources.txt"
        )
    ...
    if xml_parameters.exponential_type != "deep":
        assert False, (
            "The metric model without deep probably does not work now ! to be checked."
        )
        ...
        mode_descent_output_path = os.path.join(
            preprocessings_folder, "2_gradient_descent_on_the_mode"
        )
        ...
        estimated_alphas = np.loadtxt(
            os.path.join(mode_descent_output_path, "LongitudinalMetricModel_alphas.txt")
        )
        estimated_onset_ages = np.loadtxt(
            os.path.join(
                mode_descent_output_path, "LongitudinalMetricModel_onset_ages.txt"
            )
        )
        ...
        initial_log_acceleration_std = et.SubElement(
            model_xml, "initial-log-acceleration-std"
        )
        ...
        if xml_parameters.exponential_type == "parametric":
            interpolation_points = et.SubElement(
                deformation_parameters, "interpolation-points-file"
            )
            interpolation_points.text = os.path.join(
                mode_descent_output_path,
        ...
        concentration_of_timepoints = et.SubElement(
            deformation_parameters, "concentration-of-timepoints"
        )
        concentration_of_timepoints.text = str(
            xml_parameters.concentration_of_time_points
        )

        estimated_fixed_effects = np.load(
            os.path.join(
                mode_descent_output_path,
        ...
        if xml_parameters.exponential_type in [
            "parametric"
        ]:  # otherwise it's not saved !
            metric_parameters_file = et.SubElement(
                deformation_parameters, "metric-parameters-file"
            )
            metric_parameters_file.text = os.path.join(
                mode_descent_output_path,
                "LongitudinalMetricModel_metric_parameters.txt",
            )

        if (
            xml_parameters.number_of_sources is not None
            and xml_parameters.number_of_sources > 0
        ):
            initial_sources_file = et.SubElement(model_xml, "initial-sources")
            initial_sources_file.text = os.path.join(
                mode_descent_output_path, "LongitudinalMetricModel_sources.txt"
            )
            number_of_sources = et.SubElement(
                deformation_parameters, "number-of-sources"
            )
            number_of_sources.text = str(xml_parameters.number_of_sources)
            initial_modulation_matrix_file = et.SubElement(
                model_xml, "initial-modulation-matrix"
            )
            initial_modulation_matrix_file.text = os.path.join(
                mode_descent_output_path,
        ...
        v0.text = os.path.join(
            mode_descent_output_path, "LongitudinalMetricModel_v0.txt"
        )

        p0 = et.SubElement(deformation_parameters, "p0")
        p0.text = os.path.join(
            mode_descent_output_path, "LongitudinalMetricModel_p0.txt"
        )

        initial_onset_ages = et.SubElement(model_xml, "initial-onset-ages")
        initial_onset_ages.text = os.path.join(
            mode_descent_output_path, "LongitudinalMetricModel_onset_ages.txt"
        )

        initial_log_accelerations = et.SubElement(
            model_xml, "initial-log-accelerations"
        )
        initial_log_accelerations.text = os.path.join(
            mode_descent_output_path, "LongitudinalMetricModel_log_accelerations.txt"
        )
        ...
        deep_net_initialization_path = os.path.join(
            preprocessings_folder, "2_initialize_deep_network"
        )
        ...
        logger.info(
            "Bounding box for lsd_observations", np.max(np.abs(lsd_observations), 0)
        )
        ...
        train_dataset = TensorDataset(
            lsd_observations[:train_len], observations[:train_len]
        )
        ...
        for key in xml_parameters.template_specifications:
            ...
            def_type.text = xml_parameters.template_specifications[key][
        ...
        metric_parameters_file.text = os.path.join(
            deep_net_initialization_path, "metric_parameters.txt"
        )
        ...
        if (
            ...
            initial_sources_file = et.SubElement(model_xml, "initial-sources")
            initial_sources_file.text = os.path.join(
                deep_net_initialization_path, "sources.txt"
            )
            number_of_sources = et.SubElement(
                deformation_parameters, "number-of-sources"
            )
            ...
            initial_modulation_matrix_file.text = os.path.join(
                deep_net_initialization_path, "modulation_matrix.txt"
            )
        ...
        for elt in model_xml.iter("metric-parameters-file"):
            elt.text = os.path.join(
                "output", "LongitudinalMetricModel_metric_parameters.txt"
            )
