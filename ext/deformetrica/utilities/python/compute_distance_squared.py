...
def compute_distance_squared(
    ...
    return (
        multi_object_attachment.compute_distances(
            {
...
if __name__ == "__main__":
    ...
    if not os.path.isfile(path_to_mesh_1):
        raise RuntimeError(
            "The specified source file " + path_to_mesh_1 + " does not exist."
        )
    if not os.path.isfile(path_to_mesh_2):
        raise RuntimeError(
            "The specified source file " + path_to_mesh_2 + " does not exist."
        )
