...
if __name__ == "__main__":
    ...
    assert len(sys.argv) >= 2, (
        "Usage: " + sys.argv[0] + " <model.xml> <optional --output-dir=path_to_output>"
    )
    ...
    template_data = {
        key: torch.from_numpy(value).type(model_options["tensor_scalar_type"])
    ...
    logger.info(
        ">> Reading "
        + str(len(control_points))
        + " initial control points from file: "
        + model_options["initial_control_points"]
    )
    control_points = torch.from_numpy(control_points).type(
        model_options["tensor_scalar_type"]
    )

    # Momenta.
    momenta = read_3D_array(model_options["initial_momenta"])
    logger.info(
        ">> Reading initial momenta from file: " + model_options["initial_momenta"]
    )
    ...
    modulation_matrix = torch.from_numpy(modulation_matrix).type(
        model_options["tensor_scalar_type"]
    )
    ...
    spatiotemporal_reference_frame = SpatiotemporalReferenceFrame(
        kernel=kernel_factory.factory(
            kernel_type=model_options["deformation_kernel_type"],
    ...
    spatiotemporal_reference_frame.write(
        "SpatioTemporalReferenceFrame",
        objects_name,
