...
class FunctionalTest(unittest.TestCase):
    ...
    def run_configuration(
        ...
        model_xml,
        data_set_xml,
        optimization_parameters_xml,
        command="estimate",
        precision=DEFAULT_PRECISION,
    ):
        # Run.
        path_to_deformetrica = os.path.normpath(
            os.path.join(path_to_test, "../../../../../../deformetrica/__main__.py")
        )
        path_to_model_xml = os.path.normpath(
            os.path.join(os.path.dirname(path_to_test), model_xml)
        )
        ...
        path_to_data_set_xml = (
            os.path.normpath(os.path.join(os.path.dirname(path_to_test), data_set_xml))
            if data_set_xml is not None
            else None
        )
        path_to_output = os.path.normpath(
            os.path.join(os.path.dirname(path_to_test), output_folder)
        )
        ...
        assert os.path.isdir(path_to_output_saved), (
            "No previously saved results: no point of comparison."
        )
        ...
        if command in ["estimate"]:
            path_to_deformetrica_state = os.path.join(
                path_to_output, "deformetrica-state.p"
            )
            path_to_deformetrica_state_saved = os.path.join(
                path_to_output_saved, "deformetrica-state.p"
            )
            assert os.path.isfile(path_to_deformetrica_state_saved), (
                "The expected saved pickle dump file does not exist"
            )
            assert os.path.isfile(path_to_deformetrica_state), (
                "The test did not produce the expected pickle dump file."
            )
            self._compare_pickle_dumps(
                path_to_deformetrica_state_saved,
                path_to_deformetrica_state,
                precision=precision,
            )
            ...
            path_to_deformetrica_state = os.path.join(
                ...
                "deformetrica-state.p",
            )
            path_to_deformetrica_state_saved = os.path.join(
                path_to_output_saved,
                "5_longitudinal_atlas_with_gradient_ascent",
                "deformetrica-state.p",
            )
            assert os.path.isfile(path_to_deformetrica_state_saved), (
            ...
            self._compare_all_files(
                path_to_output_saved, path_to_output, precision=precision
            )
    ...
    def _compare_pickle_dumps(
        self,
        path_to_expected_deformetrica_state,
        path_to_actual_deformetrica_state,
        precision=DEFAULT_PRECISION,
    ):
        with (
            open(
                path_to_expected_deformetrica_state, "rb"
            ) as expected_deformetrica_state_file,
            open(
                path_to_actual_deformetrica_state, "rb"
            ) as actual_deformetrica_state_file,
        ):
            expected_deformetrica_state = pickle.load(expected_deformetrica_state_file)
            actual_deformetrica_state_saved = pickle.load(
                actual_deformetrica_state_file
            )
            self._assertStateEqual(
                expected_deformetrica_state,
                actual_deformetrica_state_saved,
                precision=precision,
            )

    def _compare_all_files(
        self,
        path_to_expected_outputs,
        path_to_actual_outputs,
        precision=DEFAULT_PRECISION,
    ):
        expected_outputs = [
            f
            for f in os.listdir(path_to_expected_outputs)
            if not f.startswith(".") and not f.endswith(".log")
        ]
        actual_outputs = [
            f
            for f in os.listdir(path_to_actual_outputs)
            if not f.startswith(".") and not f.endswith(".log")
        ]
        ...
        for fn in expected_outputs:
            ...
            if fn in ["log.txt"]:
                ...
                self._compare_txt_files(
                    path_to_expected_file, path_to_actual_file, precision=precision
                )
            elif file_extension == ".vtk":
                self._compare_vtk_files(
                    path_to_expected_file, path_to_actual_file, precision=precision
                )
    ...
    def _compare_numpy_arrays(
        self, expected, actual, rtol=DEFAULT_PRECISION, atol=DEFAULT_PRECISION
    ):
        self.assertTrue(np.allclose(expected, actual, rtol=rtol, atol=atol))

    def _compare_txt_files(
        self,
        path_to_expected_txt_file,
        path_to_actual_txt_file,
        precision=DEFAULT_PRECISION,
    ):
    ...
    def _compare_vtk_files(
        self,
        path_to_expected_vtk_file,
        path_to_actual_vtk_file,
        precision=DEFAULT_PRECISION,
    ):
        expected, expected_dimension = dfca.io.DeformableObjectReader.read_file(
            path_to_expected_vtk_file
        )
        actual, dimension = dfca.io.DeformableObjectReader.read_file(
            path_to_actual_vtk_file
        )
