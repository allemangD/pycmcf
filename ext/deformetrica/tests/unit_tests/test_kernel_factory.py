...
class KernelFactoryTest(unittest.TestCase):
    ...
    def __isKernelValid(self, instance):
        ...
        self.assertEqual(instance.kernel_width, 1.0)
...
class KernelTestBase(unittest.TestCase):
    def setUp(self):
        ...
        self.expected_convolve_res = torch.tensor(
            [
                [
                    0.442066994263886070548608131503,
                    0.674582639132687567062873768009,
                    0.823952763352665096263649502362,
                ],
                [
                    0.453613208431209002924333617557,
                    0.696584221587193019864514553774,
                    1.024381845902817111948479578132,
                ],
                [
                    0.442705483227435858673004531738,
                    0.650869945838532748538796113280,
                    1.399729801162817866000409594562,
                ],
                [
                    0.514609773097077893844186746719,
                    0.798936940519222593692916234431,
                    1.009915759134895285598076952738,
                ],
            ],
        ...
        self.expected_convolve_gradient_res = torch.tensor(
            [
                [
                    0.055114156220794276175301007470,
                    0.119300787807738090107179118604,
                    0.075544184568281505520737084680,
                ],
                [
                    0.374664655710255534160069146310,
                    -0.082597015881930607728023119307,
                    -0.241403157091644060550095218787,
                ],
                [
                    -0.512730247054557164432253557607,
                    -0.316429061213168882904511747256,
                    0.194564631495594497767598340943,
                ],
                [
                    0.082951435123507416546928538992,
                    0.279725289287361400525355747959,
                    -0.028705658972231928860452399022,
                ],
            ],
            dtype=self.torch_dtype,
        )
    ...
    def _assert_tensor_close(self, t1, t2, precision=1e-15):
        ...
        # logger.info(t1 - t2)
        self.assertTrue(
            np.allclose(t1, t2, rtol=precision, atol=precision),
    ...
    def test_multi_instance(self):
        k1 = dfca.kernels.factory(dfca.kernels.Type.TORCH, kernel_width=1.0)
        k2 = dfca.kernels.factory(dfca.kernels.Type.TORCH, kernel_width=1.0)
        k3 = dfca.kernels.factory(dfca.kernels.Type.TORCH, kernel_width=1.1)
        k4 = dfca.kernels.factory(dfca.kernels.Type.KEOPS, kernel_width=1.0)
        k5 = dfca.kernels.factory(dfca.kernels.Type.KEOPS, kernel_width=1.0)
...
class TorchKernelTest(KernelTestBase):
    ...
    def test_convolve_cpu(self):
        kernel_instance = dfca.kernels.factory(
            dfca.kernels.Type.TORCH, kernel_width=1.0
        )
    ...
    def test_convolve_gpu(self):
        kernel_instance = dfca.kernels.factory(
            dfca.kernels.Type.TORCH, gpu_mode=dfca.GpuMode.FULL, kernel_width=1.0
        )
    ...
    def test_gpu_mode(self):
        ...
        for gpu_mode in dfca.GpuMode:
            ...
            kernel_instance = dfca.kernels.factory(
                dfca.kernels.Type.TORCH, gpu_mode=gpu_mode, kernel_width=1.0
            )
            res = kernel_instance.convolve(self.x, self.y, self.p)
...
class KeopsKernelTest(KernelTestBase):
    ...
    def test_convolve_cpu(self):
        kernel_instance = dfca.kernels.factory(
            dfca.kernels.Type.KEOPS, kernel_width=1.0
        )
        res = kernel_instance.convolve(self.x, self.y, self.p)
        self._assert_tensor_close(res, self.expected_convolve_res)
    ...
    def test_convolve_gpu(self):
        kernel_instance = dfca.kernels.factory(
            dfca.kernels.Type.KEOPS, gpu_mode=dfca.GpuMode.FULL, kernel_width=1.0
        )
    ...
    def test_gpu_mode(self):
        ...
        for gpu_mode in [
            gpu_mode for gpu_mode in [gpu_mode for gpu_mode in dfca.GpuMode]
        ]:
            if gpu_mode is dfca.GpuMode.AUTO:
                continue  # TODO
            ...
            kernel_instance = dfca.kernels.factory(
                dfca.kernels.Type.KEOPS, gpu_mode=gpu_mode, kernel_width=1.0
            )
...
class KeopsVersusCuda(unittest.TestCase):
    ...
    def test_keops_and_torch_gaussian_convolve_are_equal(self):
        ...
        keops_kernel = dfca.kernels.factory(
            dfca.kernels.Type.KEOPS, kernel_width=kernel_width
        )
        torch_kernel = dfca.kernels.factory(
            dfca.kernels.Type.TORCH, kernel_width=kernel_width
        )
        random_control_points_1 = (
            torch.from_numpy(np.random.randn(number_of_control_points, dimension))
            .type(self.tensor_scalar_type)
            .requires_grad_()
        )
        random_control_points_2 = (
            torch.from_numpy(np.random.randn(number_of_control_points, dimension))
            .type(self.tensor_scalar_type)
            .requires_grad_()
        )
        ...
        keops_convolve_11 = keops_kernel.convolve(
            random_control_points_1, random_control_points_1, random_momenta_1
        )
        torch_convolve_11 = torch_kernel.convolve(
            random_control_points_1, random_control_points_1, random_momenta_1
        )
        keops_convolve_12 = keops_kernel.convolve(
            random_control_points_1, random_control_points_2, random_momenta_2
        )
        ...
        keops_total_12 = torch.dot(
            random_momenta_1.view(-1), keops_convolve_12.view(-1)
        )
        torch_total_12 = torch.dot(
            random_momenta_1.view(-1), torch_convolve_12.view(-1)
        )
        ...
        self.assertTrue(
            np.allclose(
                keops_dcp_1, torch_dcp_1, rtol=self.precision, atol=self.precision
            )
        )
        self.assertTrue(
            np.allclose(
                keops_dcp_2, torch_dcp_2, rtol=self.precision, atol=self.precision
            )
        )
        self.assertTrue(
            np.allclose(
                keops_dmom_1, torch_dmom_1, rtol=self.precision, atol=self.precision
            )
        )
        self.assertTrue(
            np.allclose(
                keops_dmom_2, torch_dmom_2, rtol=self.precision, atol=self.precision
            )
        )
    ...
    def test_keops_and_torch_varifold_convolve_are_equal(self):
        ...
        random_areas_1 = (
            torch.from_numpy(np.random.randn(number_of_control_points, 1))
            .type(self.tensor_scalar_type)
            .requires_grad_()
        )
        random_areas_2 = (
        ...
        # Compute the desired forward quantities.
        keops_convolve_11 = keops_kernel.convolve(
            (random_points_1, random_normals_1),
            (random_points_1, random_normals_1),
            random_areas_1,
            mode="varifold",
        )
        ...
        keops_convolve_12 = keops_kernel.convolve(
            (random_points_1, random_normals_1),
            (random_points_2, random_normals_2),
            random_areas_2,
            mode="varifold",
        )
        torch_convolve_12 = torch_kernel.convolve(
        ...
        [keops_dp_1, keops_dp_2, keops_dn_1, keops_dn_2, keops_da_1, keops_da_2] = (
            torch.autograd.grad(
                keops_total_12,
                [
                    random_points_1,
        ...
        [torch_dp_1, torch_dp_2, torch_dn_1, torch_dn_2, torch_da_1, torch_da_2] = (
            torch.autograd.grad(
                torch_total_12,
                [
                    random_points_1,
        ...
        self.assertTrue(
            np.allclose(
                keops_dp_1, torch_dp_1, rtol=self.precision, atol=self.precision
            )
        )
        self.assertTrue(
            np.allclose(
                keops_dp_2, torch_dp_2, rtol=self.precision, atol=self.precision
            )
        )
        self.assertTrue(
            np.allclose(
                keops_dn_1, torch_dn_1, rtol=self.precision, atol=self.precision
            )
        )
        self.assertTrue(
            np.allclose(
                keops_dn_2, torch_dn_2, rtol=self.precision, atol=self.precision
            )
        )
        self.assertTrue(
            np.allclose(
                keops_da_1, torch_da_1, rtol=self.precision, atol=self.precision
            )
        )
        self.assertTrue(
            np.allclose(
                keops_da_2, torch_da_2, rtol=self.precision, atol=self.precision
            )
        )
    ...
    def test_keops_and_torch_convolve_gradient_are_equal(self):
        ...
        # Compute the desired backward quantities.
        keops_total_12 = torch.dot(
            random_momenta_1.view(-1), keops_convolve_gradient_12.contiguous().view(-1)
        )
        torch_total_12 = torch.dot(
            random_momenta_1.view(-1), torch_convolve_gradient_12.contiguous().view(-1)
        )
        ...
        keops_convolve_gradient_11_bis = (
            keops_convolve_gradient_11_bis.detach().cpu().numpy()
        )
        torch_convolve_gradient_11_bis = (
            torch_convolve_gradient_11_bis.detach().cpu().numpy()
        )
        ...
        self.assertTrue(
            np.allclose(
                keops_convolve_gradient_11,
                torch_convolve_gradient_11,
                rtol=self.precision,
                atol=self.precision,
            )
        )
        self.assertTrue(
            np.allclose(
                keops_convolve_gradient_12,
                torch_convolve_gradient_12,
                rtol=self.precision,
                atol=self.precision,
            )
        )
