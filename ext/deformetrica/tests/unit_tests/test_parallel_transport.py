...
class ParallelTransportTests(unittest.TestCase):
    ...
    def test_parallel_transport(self):
        ...
        """
        control_points = dfca.io.read_2D_array(
            os.path.join(
                unit_tests_data_dir, "parallel_transport", "control_points.txt"
            )
        )
        momenta = dfca.io.read_3D_array(
            os.path.join(
                unit_tests_data_dir, "parallel_transport", "geodesic_momenta.txt"
            )
        )
        momenta_to_transport = dfca.io.read_3D_array(
            os.path.join(
                unit_tests_data_dir, "parallel_transport", "momenta_to_transport.txt"
            )
        )
        transported_momenta_truth = dfca.io.read_3D_array(
            os.path.join(
                unit_tests_data_dir, "parallel_transport", "ground_truth_transport.txt"
            )
        )
        ...
        momenta_torch = Variable(
            torch.from_numpy(momenta).type(self.tensor_scalar_type)
        )
        momenta_to_transport_torch = Variable(
            torch.from_numpy(momenta_to_transport).type(self.tensor_scalar_type)
        )

        factor = 5
        geodesic = dfca.deformations.Geodesic(
            dense_mode=False,
            kernel=dfca.kernels.factory(
                "keops", kernel_width=0.01, gpu_mode=dfca.GpuMode.NONE
            ),
            t0=0.0,
        ...
        transported_momenta = (
            geodesic.parallel_transport(
                momenta_to_transport_torch, is_orthogonal=False
            )[-1]
            .detach()
            .numpy()
        )
