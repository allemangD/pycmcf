...
class SurfaceMeshTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.points = [
            np.array([148.906, 136.813, 58.2132]),
            np.array([149.69, 133.984, 63.2745]),
            np.array([156.384, 134.243, 63.4685]),
        ]
    ...
    def test_read_surface_mesh(self):
        ...
        """
        surface_mesh = self._read_surface_mesh(
    ...
    def test_centers_and_normals(self):
        ...
        for i, triangle in enumerate(triangles):
            ...
            normal = (
                np.cross(
                    pts_triangle[1] - pts_triangle[0], pts_triangle[2] - pts_triangle[0]
                )
                / 2.0
            )
            self.assertTrue(np.allclose(center, centers.numpy()[i]))
            self.assertTrue(
                np.allclose(normal, normals.numpy()[i], rtol=1e-04, atol=1e-4)
            )
