...
class ShootingTests(unittest.TestCase):
    ...
    def test_geodesic_shooting(self):
        ...
        momenta = np.array([[1.0, 0.2, 0.0]])

        control_points_torch = Variable(
            torch.from_numpy(control_points).type(dfca.default.tensor_scalar_type)
        )
        momenta_torch = Variable(
            torch.from_numpy(momenta).type(dfca.default.tensor_scalar_type)
        )
        ...
        for cp, mom, time in zip(cp_traj, mom_traj, times_traj):
            self.assertTrue(
                np.allclose(cp.detach().numpy(), control_points + time * momenta)
            )
